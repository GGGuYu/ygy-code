...
跨会话消息允许同一台机器上的多个交互式 ygy-code Session 互相发现并交换纯文本工作请求。每个参与通信的根 Session 仍拥有独立的模型、对话、工作目录和本地权限边界。
...
Session 通过同一个 ygy-code 用户目录发现彼此。默认情况下，同一系统用户的 Session 共用 `~/.ygy-code`；如果设置了 `YGY_CODE_HOME`，参与通信的 Session 必须使用相同的值。
...
Peer 传输仅限本机：使用 ygy-code 用户目录下的运行时注册表和 Unix Domain Socket，不会监听网络端口。认证 token 和投递账本属于内部实现，不会暴露给模型。
...
- **This session is not a named agent**：使用 `ygy --name <名称>` 重启。
- **No other reachable sessions**：确认两端均已命名、运行于 macOS/Linux，并使用相同的 `YGY_CODE_HOME`。
- **Name is ambiguous**：从 `/list-agents` 复制精确的 `peer:<uuid>` 地址。
- **消息一直处于 held**：在 `dialogExpiryMs` 到期前到接收终端选择 Accept 或 Refuse。
- **需要诊断日志**：设置 `DEBUG_STDOUT=1` 启动；日志写入 `~/.ygy-code/logs/debug.log`。
