...
Plugins are how third-party authors extend `ygy` without forking the CLI.
...
ygy plugin install linear@anthropic-marketplace

# Install from a GitHub repo
ygy plugin install github:owner/repo

# Install from a local path (great for plugin development)
ygy plugin install ./my-plugin

# List what's installed
ygy plugin list

# Remove a plugin
ygy plugin uninstall linear@anthropic-marketplace
```
...
| ------------------- | ------------------------------ | ------------------------------------ |
| List plugins        | `/plugin list`                 | `ygy plugin list`                     |
| Show plugin details | `/plugin info <id>`            | `ygy plugin info <id>`                |
| Install             | `/plugin install <source>`     | `ygy plugin install [--yes] <source>` |
| Uninstall           | `/plugin uninstall <id>`       | `ygy plugin uninstall <id>`           |
| Enable / disable    | `/plugin enable\|disable <id>` | `ygy plugin enable\|disable <id>`     |
| Search marketplaces | `/plugin search <keyword>`     | `ygy plugin search <keyword>`         |
| Update              | `/plugin update <id\|--all>`   | `ygy plugin update <id\|--all>`       |
| Diagnose problems   | `/plugin doctor`               | `ygy plugin doctor`                   |
| Manage marketplaces | `/plugin marketplace …`        | `ygy plugin marketplace …`            |

The non-interactive form is intended for scripts and CI. `ygy plugin
...
`ygy plugin install` accepts four kinds of source:
...
`ygy plugin install` shows a preview before committing the install:
...
```bash
ygy plugin install --yes linear@anthropic-marketplace
```
...
- **userConfig prompts** — plugins with `userConfig` fields install
  without saving those values. ygy-code does not create literal `<unset>`
...
| Scope     | File                                | Notes                                    |
| --------- | ----------------------------------- | ---------------------------------------- |
| `user`    | `~/.ygy-code/settings.json`           | Default for `ygy plugin enable\|disable`. |
| `project` | `<cwd>/.ygy-code/settings.local.json` | Per-user, gitignored. Overrides `user`.  |
...
ygy plugin disable linear@anthropic-marketplace --scope=project
# Enable in user scope (the default)
ygy plugin enable linear@anthropic-marketplace --scope=user
```
...
Everything plugin-related lives under `~/.ygy-code/plugins/`:
...
```bash
ygy --no-plugins    # skip plugin discovery entirely
ygy --no-hooks      # load plugins but skip hook execution
```
...
Values are stored in `~/.ygy-code/plugins/user-config.json` with file mode
...
for CI, hand-write `~/.ygy-code/plugins/user-config.json` before the
install. **The `/plugin install` slash command also skips this**
(it runs as if `--yes` were passed), so install userConfig-bearing
plugins with `ygy plugin install` instead — see "Limitations of the
...
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| Plugin doesn't appear after install     | Run `/plugin refresh` (inside an interactive session) or restart `ygy`. Contributions bind on the registry. |
...
| Hook fires unexpectedly                 | Set `DEBUG_STDOUT=1`, restart, then `tail ~/.ygy-code/logs/debug.log` and grep `hooks.`.                     |
| Suspect a plugin is breaking everything | Launch with `ygy --no-plugins`. If the problem disappears, isolate with `/plugin disable <id>` + refresh.   |
| Hook is slow / hangs                    | Launch with `ygy --no-hooks`. Each hook also has a 5s default timeout.                                      |

---

## Compatibility with Claude Code plugins

`ygy` deliberately reads `.claude-plugin/plugin.json` in addition to its
native `.ygy-code-plugin/plugin.json` path. It supports common Claude Code
...
variable syntax follows ygy-code's MCP rules. After installing a third-party
...
plugin must provide one of ygy-code's accepted manifest paths and use the
