...
ygy-code ships with a built-in MCP client. Any MCP-protocol server you
...
Add an `mcpServers` field to `~/.ygy-code/config.json` (create the file if
needed), then restart `ygy`:
...
| ------- | --------------------------- | ------------------------------------------------ |
| User    | `~/.ygy-code/config.json`     | Personal-use servers (filesystem, github, etc.)  |
| Project | `<cwd>/.ygy-code/config.json` | Servers for the directory where `ygy` is launched |
...
`~/.ygy-code/trusted-projects.json`.

> **Windows paths**: `~/.ygy-code` maps to `%USERPROFILE%\.ygy-code` on
...
`~/.ygy-code/mcp/tokens/<server>.json`. Subsequent launches inject
...
| Restarting `ygy` feels slow                | `/mcp refresh` reconnects in place — no CLI restart                                                                                             |
| Project config not loading                | Did you decline the trust dialog at startup? Remove the matching path from `~/.ygy-code/trusted-projects.json` and restart to re-decide           |
| Want to temporarily skip a server         | Set `enabled: false` on the entry — more visible than commenting out the whole block                                                            |

With `DEBUG_STDOUT=1` set, MCP events land in `~/.ygy-code/logs/debug.log`.
...
ygy-code supports the common Claude Code `mcpServers` fields documented
above. Compatible entries can be copied into `~/.ygy-code/config.json`, but the
schemas are not identical: ygy-code accepts only `command` / `args` / `env` /
