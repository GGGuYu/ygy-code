...
`ygy-code` doesn't run its own marketplace. It **subscribes** to other
...
When `ygy` starts up **or you run any `ygy plugin …` subcommand**, it ensures this subscription exists:
...
anthropic-marketplace`, the next CLI startup or `ygy plugin …` command
...
`ygy` follows the common public Claude Code marketplace shape. The
...
> running `ygy plugin marketplace add <alias> <source>` is what ygy-code
...
`ygy` accepts the source forms listed below, covering the common forms used by
...
deletes the clone. Subsequent `ygy plugin marketplace refresh <name>`
re-clones.
...
`~/.ygy-code/plugins/marketplaces/<name>/marketplace.json`. After the first
...
After `ygy plugin marketplace refresh <name>`, the index lives at
`~/.ygy-code/plugins/marketplaces/<name>/marketplace.json`. The cache
...
`ygy plugin …` command restores it. There is currently no persisted opt-out
...
forms described above, so `ygy` can read it directly. Third-party catalogs and
...
are ignored; their features do not activate in `ygy`.
...
| Want to migrate from Claude Code's marketplace | It's already subscribed by default. Just `ygy plugin install <name>@anthropic-marketplace`.                     |
