...
| `${pluginDataDir}` | 插件**持久数据目录**（`~/.ygy-code/plugins/data/<id>/`），跨重装/升级保留——索引、缓存、用户偏好放这里。第一次替换时自动 `mkdir -p`，hook 直接写就行 |
| `${cwd}`           | 事件发生时的当前工作目录                                                                                                                          |
| `${homedir}`       | `os.homedir()`                                                                                                                                    |
...
Hook 崩溃、超时或非零退出，默认按 `allow` 处理，warning 写到 `~/.ygy-code/logs/debug.log`（要先 `DEBUG_STDOUT=1` 或 `ygy --plugin-debug` —— 后者只镜像 `plugins.` / `hooks.` / `marketplace.` 标签的行，更安静）。
...
`ygy --plugin-debug` 把这些行实时镜像到 stderr，跑一会儿 `/exit` 然后 grep `~/.ygy-code/logs/debug.log` 也行。Hook 不触发的最常见原因：(a) plugin 没启用（`/plugin list` 看），(b) 没 refresh（`/plugin refresh`），(c) `matcher` 正则不匹配工具名（工具名是 camelCase，如 `writeFile`、`edit`）。
...
**注意递归**：hook 自己又调 `ygy` 或起 agent 时，那些工具调用也会触发 `PreToolUse`。hook 逻辑要紧凑，多用 `matcher` 正则约束触发的 tool。
...
```bash
ygy --no-hooks            # 插件正常加载，hooks 不执行
ygy --no-plugins          # 核弹：完全关插件
```
