...
ygy-code supports sub-agent delegation through the `task` tool: the model
...
- Config: `"browser": { "enabled": true }` in `~/.ygy-code/config.json`
...
cookies. No extension is required. If two `ygy` instances in the same workspace
...
silently switching to a temporary profile and losing signed-in state. ygy-code
...
- After visual web changes, the default-enabled `browserVisualCheck` opens a
  ...
  `.ygy-code/sessions/*.jsonl`; occasional MCP output files live only under the
...
| Scope   | Path                             |
| ------- | -------------------------------- |
| User    | `~/.ygy-code/agents/<name>.md`     |
| Project | `<cwd>/.ygy-code/agents/<name>.md` |
...
> **Windows paths**: `~/.ygy-code` maps to `%USERPROFILE%\.ygy-code`.
