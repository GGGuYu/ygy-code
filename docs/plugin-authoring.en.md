...
schema reference plus the layout conventions ygy-code expects.
...
1. `.ygy-code-plugin/plugin.json` ← preferred for new plugins
...
Only `name` is required. If `version` is omitted, ygy-code uses `0.0.0` for
...
```jsonc
{
  ...
  // `{ "mcpServers": { ... } }` (same as ~/.ygy-code/config.json),
  ...
  // At install time (`ygy plugin install <src>` without `--yes`) the CLI
  // walks each field and prompts for a value, writing the result to
  // ~/.ygy-code/plugins/user-config.json (mode 0600). At runtime each value
  ...
  // must be installed via `ygy plugin install`; see plugins.en.md's slash
  // command limitations section.

  // ── Dependencies & engines ─────────────────────────────────────
  "dependencies": ["base-skills@anthropic-marketplace"],
  "engines": { "ygy-code": ">=0.5.0" },
}
```
...
- **`skills`** / **`agents`** / **`commands`** — paths to the
  ...
  `~/.ygy-code/commands/<name>.md` (user scope) or
  `<cwd>/.ygy-code/commands/<name>.md` (project scope) — precedence is
...
- **`mcpServers`** — path or inline object. When unset, the loader
  ...
  Per-server schema follows `~/.ygy-code/config.json`; environment references
  ...
  `${NAME}` entries. ygy-code does not inject a plugin-root variable into MCP
...
2. `ygy plugin install ./my-plugin` — copies into
   `~/.ygy-code/plugins/cache/local/<name>/<version>/` and records the
   install.
3. In an open session, run `/plugin refresh` to pick up contributions; starting
   a new `ygy` session also loads them.
4. Iterate. Re-running `ygy plugin install ./my-plugin` over the same
...
| Hook never fires                            | Run `/plugin refresh` after install (in-session hot reload, no restart needed) — or restart `ygy`. Verify via `ygy --plugin-debug` and look for `hooks.exec-ran` entries in `~/.ygy-code/logs/debug.log`.                                                                   |
...
| `${pluginDataDir}` write fails              | Auto-created at `~/.ygy-code/plugins/data/<sanitised-plugin-id>/`, preserved across versions. First substitution does `mkdir -p`; permission errors surface in the shell. **Don't** write persistent data to `${pluginDir}` — it gets wiped on every reinstall / upgrade. |
...
| Want to release publicly                    | Publish to a marketplace.json that lists your plugin's git URL; tell users to `ygy plugin marketplace add <name> <source>`. See [marketplace.en.md](marketplace.en.md).                                                                                                  |
