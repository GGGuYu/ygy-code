...
ygy-code 不自己运营 marketplace，只**订阅**其他 marketplace。它支持常见的 Claude Code marketplace schema 与 source 形式，包括 Anthropic 官方目录。
...
ygy-code 启动 CLI **或运行任何 `ygy plugin …` 子命令时**，都会确保下面这条订阅存在：

| 名字                    | 源                                          | 说明                                           |
| ----------------------- | ------------------------------------------- | ---------------------------------------------- |
| `anthropic-marketplace` | `github:anthropics/claude-plugins-official` | Anthropic 官方 Claude Code marketplace，保留名 |

如果用 `/plugin marketplace remove anthropic-marketplace` 删除它，下次启动 CLI 或执行 `ygy plugin …` 时会再次添加（详见 [幂等性](#幂等性)）。
...
ygy plugin marketplace add community github:foo/ygy-code-marketplace

# 从直接服务 marketplace.json 的 HTTPS URL
ygy plugin marketplace add internal https://intranet.example.com/plugins.json

# 然后拉它的索引
ygy plugin marketplace refresh community
```

列出订阅：

```bash
ygy plugin marketplace list
# →
# Subscribed marketplaces (2):
#   anthropic-marketplace github:anthropics/claude-plugins-official [official]
#   community             github:foo/ygy-code-marketplace
```

看某个订阅里有啥：

```bash
ygy plugin marketplace info community
ygy plugin search linear
```

取消订阅：

```bash
ygy plugin marketplace remove community
```
...
| `ygy-code-official`       | `github:GGGuYu/…`     |

用保留名但源不对的订阅会被 API 直接拒：

```bash
$ ygy plugin marketplace add anthropic-marketplace github:bad/marketplace
...
ygy-code 遵循常见的公开 Claude Code marketplace 结构。repo 内首选 **`.claude-plugin/marketplace.json`**，也接受根目录的 `marketplace.json` 作为 fallback。
...
```bash
ygy plugin marketplace add <name> github:youruser/yourrepo
```

CLI 浅克隆 → 优先读 `.claude-plugin/marketplace.json`、缺失时回退到 repo 根的 `marketplace.json` → 缓存、删克隆。之后 `ygy plugin marketplace refresh <name>` 再克隆刷新。
...
```bash
ygy plugin marketplace add <name> https://example.com/marketplace.json
```

CLI 用 `fetch()` 拉取。适合企业内部 marketplace——可按 VPN 服务不同索引、强制 TLS 等。

两种形态都会先验证索引，再把原始 JSON 缓存到 `~/.ygy-code/plugins/marketplaces/<name>/marketplace.json`。首次 refresh 后可以离线浏览缓存的索引；安装远程插件仍可能需要访问它的 source。

---

## 缓存

`ygy plugin marketplace refresh <name>` 之后索引在 `~/.ygy-code/plugins/marketplaces/<name>/marketplace.json`。缓存文件的 mtime 作为"新鲜度"标记——目前**没有自动 TTL 刷新**，需要用户（或脚本）主动 `refresh` 拉新。
...
`ensureDefaultMarketplaces()` 只检查有没有名为 `anthropic-marketplace` 的条目：已存在就不改；不存在就补上，同时保留其他订阅。因此单独删除默认条目只是临时的，下次启动 CLI 或执行 `ygy plugin …` 时会恢复。目前在启用插件系统的情况下没有持久化 opt-out。

---

## 与 Claude Code 的兼容性

Anthropic 官方 Claude Code marketplace 使用了上文的常见字段和 source 形式，ygy-code 可以直接读取。第三方目录或插件可能使用额外的 Claude Code 能力，安装后应通过 `/plugin info`、`/plugin doctor` 与 `/mcp list` 检查。插件 manifest 的探测顺序是：

- `.ygy-code-plugin/plugin.json`
- `.claude-plugin/plugin.json`
- `plugin.json`

未知顶层 manifest 字段（包括 `output-styles`、`lspServers`）会被忽略，对应能力不会在 ygy-code 中激活。
...
| 想从 Claude Code marketplace 迁过来 | 它默认就订阅了。`ygy plugin install <name>@anthropic-marketplace` 即可                      |
