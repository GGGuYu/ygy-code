...
| Scope | 路径                                   | 何时用            |
| ----- | -------------------------------------- | ----------------- |
| 用户  | `~/.ygy-code/skills/<name>/SKILL.md`     | 个人通用工作流    |
| 项目  | `<cwd>/.ygy-code/skills/<name>/SKILL.md` | `ygy` 启动目录使用 |

项目级同名 skill 会覆盖 plugin 和用户级同名 skill。本仓库根目录的 `.ygy-code/` 已被 gitignore；团队共享 skill 建议发布成 plugin（见 [plugins.md](./plugins.md)）。
...
- user → `~/.ygy-code/settings.json` 的 `disabledSkills` 数组
- project → `<cwd>/.ygy-code/settings.local.json` 的 `disabledSkills` 数组
...
| 问题                       | 处理                                                    |
| -------------------------- | ------------------------------------------------------- |
| 改了 SKILL.md 不生效       | `/skill refresh` 或重启 `ygy`                            |
