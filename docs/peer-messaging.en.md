...
Cross-session messaging lets interactive ygy-code sessions on the same machine discover one another and exchange plain-text work requests. Each participating root session keeps its own model, conversation, working directory, and local permission boundary.
...
Sessions discover peers through the same ygy-code user directory. With the default setup, sessions running as the same OS user share `~/.ygy-code`; if `YGY_CODE_HOME` is set, they must use the same value.
...
Configure the receiving policy in `~/.ygy-code/config.json`:
...
Peer transport is local-only: it uses a runtime registry and Unix-domain sockets under the ygy-code user directory, not a network listener. Authentication tokens and delivery ledgers are implementation details and are never model-visible.
...
- **This session is not a named agent** — restart it with `ygy --name <name>`.
- **No other reachable sessions** — verify that both sessions are named, run on macOS/Linux, and share the same `YGY_CODE_HOME`.
...
- **Need diagnostics** — launch with `DEBUG_STDOUT=1`; details go to `~/.ygy-code/logs/debug.log`.
