...
| 操作             | 交互内 slash 命令              | 命令行                               |
| ---------------- | ------------------------------ | ------------------------------------ |
| 列出插件         | `/plugin list`                 | `ygy plugin list`                     |
| 看插件详情       | `/plugin info <id>`            | `ygy plugin info <id>`                |
| 安装             | `/plugin install <source>`     | `ygy plugin install [--yes] <source>` |
| 卸载             | `/plugin uninstall <id>`       | `ygy plugin uninstall <id>`           |
| 启用 / 禁用      | `/plugin enable\|disable <id>` | `ygy plugin enable\|disable <id>`     |
| 搜索 marketplace | `/plugin search <keyword>`     | `ygy plugin search <keyword>`         |
| 升级             | `/plugin update <id\|--all>`   | `ygy plugin update <id\|--all>`       |
| 诊断             | `/plugin doctor`               | `ygy plugin doctor`                   |
| 管理 marketplace | `/plugin marketplace …`        | `ygy plugin marketplace …`            |

`ygy plugin` 命令行形式适合脚本和 CI。`ygy plugin install` 默认走 y/N consent 提示——加 `--yes` 跳过。
...
`ygy plugin install` 在真正落盘前会显示预览：
...
- **userConfig prompt**：带 `userConfig` 字段的插件不会保存这些值。ygy-code 不会创建字面量 `<unset>`；已有的进程或 server 环境仍可能提供同名变量
...
| Scope     | 路径                                | 说明                                     |
| --------- | ----------------------------------- | ---------------------------------------- |
| `user`    | `~/.ygy-code/settings.json`           | `ygy plugin enable\|disable` 的默认 scope |
| `project` | `<cwd>/.ygy-code/settings.local.json` | per-user 在该 repo 的覆盖，gitignored    |
...
所有插件相关的东西都在 `~/.ygy-code/plugins/` 下：
...
> **Windows 路径**：`~/.ygy-code` 在 Windows 上是 `%USERPROFILE%\.ygy-code`。
...
```bash
ygy --no-plugins    # 完全跳过插件发现
ygy --no-hooks      # 插件正常加载，但 hooks 全部不执行
```
...
`--all` 顺序跑、单条失败不影响其它（skip-on-error），结束打印汇总：`Summary: N updated, M unchanged, K failed.`。**裸跑 `ygy plugin update` 会被拒**——必须显式 `<id>` 或 `--all`，避免误操作把所有插件重克隆一遍（行业惯例同 Gemini CLI 的 `extensions update --all`）。
...
值存到 `~/.ygy-code/plugins/user-config.json`，文件权限 `0600`（仅本用户可读）。Hook 跑的时候、plugin-contributed MCP server 启动的时候，这些值会自动注入到子进程 `env`，key 名就是 manifest 里的 `key` 字段。所以 hook 脚本里直接 `process.env.LINEAR_API_KEY` 就能用，无需额外胶水。
...
`--yes` 安装跳过 prompt，user-config 留空。需要 CI 装的话，提前手写 `~/.ygy-code/plugins/user-config.json` 即可。**`/plugin install` slash 命令也跳过**（等同 `--yes`），所以 userConfig 类插件请用 `ygy plugin install` 命令行版安装——详见上文"Slash 命令 `/plugin install` 的限制"。
...
| 症状                            | 先试                                                                         |
| ------------------------------- | ---------------------------------------------------------------------------- |
| 安装后插件不出现                | 跑 `/plugin refresh`（在交互内）或重启 `ygy`。贡献绑定在 registry 上          |
| `/plugin doctor` 报 load errors | 看它打印的路径——通常是 manifest 拼写错                                       |
| 插件的 MCP 服务连不上           | `/mcp list`——插件提供的 server 也会列在那里                                  |
| Hook 触发意外                   | `DEBUG_STDOUT=1` 重启 → `tail ~/.ygy-code/logs/debug.log` 搜 `hooks.`          |
| 怀疑某插件搞砸                  | `ygy --no-plugins` 启动；若问题消失就用 `/plugin disable <id>` + refresh 排查 |
| Hook 跑得慢 / 卡住              | `ygy --no-hooks` 启动；每个 hook 默认 5s 超时                                 |

---

## 与 Claude Code 插件的兼容性

ygy-code 同时识别 `.claude-plugin/plugin.json` 和原生的 `.ygy-code-plugin/plugin.json`。使用本文 schema 时，常见 Claude Code 贡献——skills、agents、slash commands、MCP servers、hooks——可以接入，但不承诺完全兼容。未知顶层字段（如 `output-styles`、`lspServers`）会被忽略，MCP 变量写法也以 ygy-code 规则为准。安装第三方插件后，用 `/plugin info`、`/plugin doctor` 与 `/mcp list` 验证。

Codex 原生的 `.codex-plugin/plugin.json` 不在探测路径中。只提供 Codex manifest 的插件需要补充 ygy-code 接受的 manifest 路径，并使用受支持的贡献 schema。
