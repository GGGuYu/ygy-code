...
2. Review [`packages/cli/tests/e2e/README.md`](../../packages/cli/tests/e2e/README.md), the selected scenarios, and their prompts/tool permissions. Most scenarios use isolated temporary directories and `YGY_CODE_HOME`, but the child process inherits the launching environment and some scenarios run with `--trust`.
