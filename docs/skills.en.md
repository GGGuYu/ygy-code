...
mkdir -p ~/.ygy-code/skills/code-review
cat > ~/.ygy-code/skills/code-review/SKILL.md <<'EOF'
---
name: code-review
...
# Restart ygy, or `/skill refresh` in the running session.
# Then in chat:
...
| Scope   | Path                                   | When                                     |
| ------- | -------------------------------------- | ---------------------------------------- |
| User    | `~/.ygy-code/skills/<name>/SKILL.md`     | Personal workflows that apply everywhere |
| Project | `<cwd>/.ygy-code/skills/<name>/SKILL.md` | Workflows for the launch directory       |

A project-level skill overrides a plugin or user-scope skill of the same name.
This repository gitignores its root `.ygy-code/`; to share skills with a team,
publish them as a plugin (see [plugins.en.md](./plugins.en.md)).

> **Windows paths**: `~/.ygy-code` maps to `%USERPROFILE%\.ygy-code` on Windows.
> Not repeated below.
...
- user → `disabledSkills` array in `~/.ygy-code/settings.json`
- project → `disabledSkills` array in `<cwd>/.ygy-code/settings.local.json`
...
`~/.ygy-code/skills/pr-review/SKILL.md`:
...
`~/.ygy-code/skills/pr-review/references/api-allowlist.md`:
...
`~/.ygy-code/skills/perf-chart/SKILL.md`:
...
| Symptom                                       | Fix                                                                    |
| --------------------------------------------- | ---------------------------------------------------------------------- |
| Edited SKILL.md but nothing changed           | `/skill refresh` or restart `ygy`                                       |
