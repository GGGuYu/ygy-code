...
ygy-code combines hand-maintained project knowledge with global long-term memory. Project rules live in the stable system prompt, while detailed memories are retrieved only when relevant instead of filling every request with the entire history.
...
On Windows, `~/.ygy-code` maps to `%USERPROFILE%\.ygy-code`. Set `YGY_CODE_HOME` to override the user directory, which is useful for isolated testing.
...
Use this for shared architecture, commands, and constraints. In a monorepo, ygy-code loads the full chain from the repository root to the current directory, allowing leaf files to override root rules.
...
Repository relationships are stored in each topic's `applies_to` metadata and influence ranking. ygy-code no longer creates project-level auto-memory files.
...
The complete source text lives in `~/.ygy-code/memory/topics/*.md`. You may edit it directly, but a running CLI does not use `fs.watch`: run `/memory reload` or restart after editing.
...
- `~/.ygy-code/memory/auto.md` and `<repo>/.ygy-code/memory/auto.md` are never read.
