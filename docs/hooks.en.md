...
| `${pluginDataDir}` | The plugin's **persistent data dir** (`~/.ygy-code/plugins/data/<id>/`). Survives uninstall+reinstall and version upgrades — use for indexes, caches, user prefs. |
...
```jsonc
{
  ...
  "plugin": {
    ...
    "dir": "/abs/.ygy-code/plugins/cache/anthropic-marketplace/linear/1.2.0",
  },
...
by default and a warning lands in `~/.ygy-code/logs/debug.log` (set
`DEBUG_STDOUT=1` to enable that log, or use `ygy --plugin-debug` which
...
`ygy --plugin-debug` mirrors these to stderr in real time; alternately
run a session and grep `~/.ygy-code/logs/debug.log` afterwards. Most
...
Beware recursion: if a hook itself invokes `ygy` or another agent, its
...
```bash
ygy --no-hooks            # plugins still load, hooks just don't run
ygy --no-plugins          # nuclear option: no plugin loading at all
```
