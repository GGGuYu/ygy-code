// @ygy-code/cli — yargs argument definitions for the `ygy` CLI.
//
...
export async function parseCliArgs(args: string[] = hideBin(process.argv)) {
  return yargs(args)
    ...
    .option('model', {
      alias: 'm',
      type: 'string',
      describe: 'Model to use (e.g. sonnet, deepseek, openai:gpt-5.6-sol)',
    })
    .option('trust', {
      alias: 't',
      type: 'boolean',
    ...
    .option('plugin-debug', {
      ...
      // log file, so you can see them live without tailing ~/.ygy-code/logs/.
      // Equivalent to setting `YGY_PLUGIN_DEBUG=1`. Doesn't change behaviour
      // — only changes where the breadcrumbs go.
      describe: 'Mirror plugin / hook / marketplace debug breadcrumbs to stderr (also YGY_PLUGIN_DEBUG=1).',
    })
    .option('continue', {
      alias: 'c',
      type: 'boolean',
    ...
    .option('resume', {
      alias: 'r',
      type: 'string',
      // Optional value: `ygy --resume` (no value) opens the picker; `ygy
