// @ygy-code/cli — Startup-time stderr messages (no API key, no web search,
...
import { PROVIDER_DETECTION_ORDER, PROVIDER_KEY_URLS, userYgyDir } from '@ygy-code/core'
...
export function printNoApiKeyMessage(): void {
  ...
  if (hint) console.error(`  ${comment(hint)}  ${code('ygy')}`)
...
export function printResumeHint(): void {
  ...
  if (!info) return
  const cmd = chalk.cyan(`ygy --resume ${info.sessionId}`)
  const dim = chalk.gray
...
const NPM_REGISTRY_URL = 'https://registry.npmjs.org/@ygy-code/cli/latest'

function updateCheckCachePath(): string {
  return path.join(userYgyDir(), 'cache', 'update-check.json')
}
...
function printUpdateHint(current: string, latest: string): void {
  const msg =
    ...
    ` ${chalk.green(latest)}` +
    chalk.gray('  Run ') +
    chalk.cyan('npm install -g @ygy-code/cli') +
