// @ygy-code/cli — Root App component
...
import type { AgentOptions, CacheMissSummary, DiffStats, LanguageModel, LoadedSession } from '@ygy-code/core'
import type { SkillDefinition, StepStats, TokenUsage, UsageBreakdown } from '@ygy-code/core'
...
interface AppProps {
  ...
  /** Pre-loaded session from `ygy --continue`. Hydrates the agent on
   ...
   *  `ygy --resume` flag path. Once Ink is ready (so askQuestion can
...
export function App({
  ...
  // resetTerminal to print "Resume: ygy --resume <id>" to the shell.
  ...
  // the CLI entry:
  //   - initialSession set: `ygy -c` already loaded the most recent
  ...
  //   - resumeIntent === 'pick': `ygy -r` wants the picker. We pop the
   ...
   *       minimum).
   *
   * Persisted to ~/.ygy-code/config.json so the choice survives restarts.
