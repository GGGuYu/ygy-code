// @ygy-code/cli — /skill slash command handler.
//
...
import type { AgentOptions, SkillDefinition, SkillSettingsScope } from '@ygy-code/core'
...
export function createSkillCommandHandler(deps: SkillCommandDeps) {
  ...
  async function handleSkill(text: string, arg: string): Promise<void> {
    ...
    if (sub === 'install') {
      ...
      const skillDir = path.join(USER_YGY_DIR, 'skills', name)
      ...
      addCommandMessage(
        text,
        `Skill **${name}** installed to \`${skillFile}\`\nRun \`/skill refresh\` to use \`/${name}\` now, or restart ygy.`,
      )
      return
    }
    ...
    if (sub === 'list') {
      ...
      if (skills.length === 0) {
        const skillsPath = path.join(USER_YGY_DIR, 'skills', '<name>', 'SKILL.md')
        addCommandMessage(
          text,
    ...
    if (sub === 'uninstall') {
      ...
      const baseDir = entry.source === 'user' ? USER_YGY_DIR : path.join(process.cwd(), '.ygy-code')
