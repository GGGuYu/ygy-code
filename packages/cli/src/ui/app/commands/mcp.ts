// @ygy-code/cli — /mcp slash command handler family.
//
...
import type { AgentOptions, ConfigScope, McpServerConfig } from '@ygy-code/core'
...
export function createMcpCommandHandler(deps: McpCommandDeps) {
  ...
  async function persistAndAnnounceNewServer(
    ...
    if (scope === 'project') lines.push('Tip: commit `.ygy-code/config.json` to share with collaborators.')
    lines.push('Run /mcp refresh to load it now, or restart ygy.')
  ...
  async function handleMcpRemove(text: string, subArgRaw: string): Promise<void> {
    ...
    addCommandMessage(
      ...
      [
        ...
        'Current session unchanged — the running server (if any) keeps working until ygy exits.',
    ...
    switch (sub) {
      ...
      case 'list': {
        ...
        if (statuses.length === 0) {
          addCommandMessage(text, 'No MCP servers configured. Add `mcpServers` to ~/.ygy-code/config.json then restart.')
          return
        }
      ...
      case 'auth': {
        ...
        if (!registry) {
          addCommandMessage(text, 'No MCP servers configured. Add `mcpServers` to ~/.ygy-code/config.json first.')
          return
        }
