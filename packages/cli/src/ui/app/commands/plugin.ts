// @ygy-code/cli — /plugin slash command handler family.
//
...
import type { AgentOptions, PluginScope } from '@ygy-code/core'
...
export function createPluginCommandHandler(deps: PluginCommandDeps) {
  ...
  function pluginDoctor(text: string): void {
    ...
    lines.push(
      '',
      '_For deeper diagnostics (mcp collisions, hook errors, unsupported `commands` contributions), set `DEBUG_STDOUT=1` and check `~/.ygy-code/logs/debug.log`._',
    )
    addCommandMessage(text, lines.join('\n'))
  }
