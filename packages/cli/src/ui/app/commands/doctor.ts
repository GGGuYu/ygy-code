// @ygy-code/cli — /doctor slash command handler.
//
...
// but scoped to what ygy-code actually needs.
import { getAvailableProviders, getEnvVarName, getOpenAIAuthStatus } from '@ygy-code/core'
import type { AgentOptions } from '@ygy-code/core'
  ...
  function handleDoctor(text: string): void {
    ...
    // ── Providers ──────────────────────────────────────────────────────
    {
      ...
      if (configuredCount === 0) {
        lines.push('', '⚠ No provider configured. Run `ygy login` or set a provider API key.')
      }
      sections.push(lines.join('\n'))
    }
