// @ygy-code/cli — Input history (Up/Down recall) backing store.
//
// Persisted form: `.ygy-code/history.jsonl`, one JSON object per line, append-
// only. Project-local so two unrelated projects don't pollute each other's
// recall — matches our existing convention (`.ygy-code/sessions/`,
// `.ygy-code/plans/`, `.ygy-code/memory/`). Claude Code instead uses a single
...
// the rest of `.ygy-code/` — gitignored, scoped, throwaway-with-the-project.
//
...
import { ensureProjectStorageDir } from '@ygy-code/core'

import type { PastedContents } from './paste-refs.js'
import type { InputAction } from './reducer.js'

const HISTORY_FILE = '.ygy-code/history.jsonl'
