// @ygy-code/cli — Agent state management hook
...
export function useAgent(initialModel: LanguageModel, options: AgentOptions, initialSession?: LoadedSession | null) {
  ...
  // ~/.ygy-code/config.json on launch).
  ...
  const pendingEditDiffsRef = useRef<Map<string, import('@ygy-code/core').EditDiffPayload>>(new Map())
  ...
  const compact = useCallback(async () => {
    ...
    let compressionApi: typeof import('@ygy-code/core')
    try {
      compressionApi = await import('@ygy-code/core')
    } catch (error) {
      lease.release()
      throw error
    }
