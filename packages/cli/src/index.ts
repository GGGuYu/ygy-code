...
// downgrades, etc. Captured to ~/.ygy-code/logs/debug.log when DEBUG_STDOUT
...
function checkNodeVersion(): void {
  ...
  if (
    ...
    console.error(
      `Error: ygy-code requires Node.js >= ${MIN_NODE_VERSION.join('.')}, but you are running ${process.versions.node}.\n` +
...
async function main() {
  ...
  // parses the rest of argv — otherwise `ygy plugin install ./foo`
  ...
  // Parse CLI arguments
  // `ygy login` is an onboarding entry point: after successful interactive
  // authentication, consume its auth-only arguments and continue into the
  // product instead of making the user run a second `ygy` command.
  ...
  // tailing ~/.ygy-code/logs/debug.log. Install BEFORE ensureDefaultMarketplaces
  ...
  if (argv['plugin-debug'] || process.env.YGY_PLUGIN_DEBUG === '1') {
  ...
  //      ("Resume: ygy --resume <id>") feeds back into this branch.
  ...
  if (argv.continue) {
    ...
    if (argv.resume === '') {
      ...
      if (!filePath) {
        console.error(
          `Error: no session found matching "${argv.resume}". Run \`ygy --resume\` to pick from the list, or \`ygy -c\` for the most recent.`,
        )
        process.exit(1)
      }
  ...
  const forcedSearchProvider = process.env.YGY_CODE_WEB_SEARCH_PROVIDER?.trim()
  if (forcedSearchProvider) {
    ...
    if (!resolveWebSearchProvider(modelId.split(':')[0])) {
      console.error(
        `Warning: YGY_CODE_WEB_SEARCH_PROVIDER=${forcedSearchProvider} is not usable ` +
