// @ygy-code/cli — Non-interactive plugin subcommands
//
// `ygy plugin <subcommand> ...` entry point — runs without mounting the
...
// way `ygy plugin install ./foo` doesn't get treated as a prompt the
// agent should answer.
...
import type { ConsentPreview, PluginScope } from '@ygy-code/core'
...
export async function runPluginCli(args: string[]): Promise<number> {
  ...
  // `ygy plugin marketplace list` (or `search`) before ever launching
...
function printUsage(): void {
  console.error(
    [
      'Usage: ygy plugin <subcommand> [args...]',
      '',
      'Subcommands:',
      ...
      '',
      'Example:',
      '  ygy plugin marketplace add anthropic-marketplace github:anthropics/marketplace',
      '  ygy plugin marketplace refresh anthropic-marketplace',
...
async function cliInfo(args: string[]): Promise<number> {
  const id = args[0]
  if (!id) {
    console.error('Usage: ygy plugin info <id>')
    return 1
  }
...
async function cliInstall(args: string[]): Promise<number> {
  ...
  if (!raw) {
    console.error('Usage: ygy plugin install [--yes] <source>')
  ...
  try {
    const result = await installPlugin({
      ...
      // values by editing ~/.ygy-code/plugins/user-config.json directly
      // or by writing them via the future `ygy plugin configure` cmd).
    ...
    // No "restart ygy" hint — this subcommand is a one-shot from the
    // shell, not running inside a live session. A user with an active
    // ygy TUI elsewhere can run `/plugin refresh` there to pick this up.
    return 0
  } catch (err) {
...
async function promptUserConfig(
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    process.stderr.write(
      chalk.yellow(
        'No TTY — skipping userConfig prompt. Pre-seed values in ~/.ygy-code/plugins/user-config.json or use --yes.\n',
      ),
    )
    return {}
  }
    ...
    case 'plugin-not-found':
      console.error(
        `Plugin '${result.pluginName}' not found in marketplace '${result.marketplace}'. ` +
          `Run 'ygy plugin marketplace refresh ${result.marketplace}' or check the spelling.`,
      )
      break
...
async function cliUninstall(args: string[]): Promise<number> {
  const id = args[0]
  if (!id) {
    console.error('Usage: ygy plugin uninstall <id>')
    return 1
  }
...
async function cliToggle(args: string[], enable: boolean): Promise<number> {
  ...
  if (!id) {
    console.error(`Usage: ygy plugin ${enable ? 'enable' : 'disable'} <id> [--scope=user|project]`)
    return 1
  }
...
async function cliUpdate(args: string[]): Promise<number> {
  // Two modes — explicit:
  //   ygy plugin update <id>   single plugin (existing behavior)
  //   ygy plugin update --all  every installed plugin, sequential, skip-on-error
  //
  // Bare `ygy plugin update` is rejected on purpose. npm makes bare = all but
  ...
  if (all && positional.length > 0) {
    console.error('ygy plugin update: pass either `--all` or a plugin id, not both.')
    return 1
  }
  if (!all && positional.length === 0) {
    console.error('Usage: ygy plugin update [--yes] <id> | --all')
    console.error('  <id>: a name@marketplace from `ygy plugin list`')
...
async function cliSearch(args: string[]): Promise<number> {
  ...
  if (!kw) {
    console.error('Usage: ygy plugin search <keyword>')
    return 1
  }
  ...
  if (marketplaces.length === 0) {
    ...
    if (km.marketplaces.length === 0) {
      console.error('No subscribed marketplaces. Add one with `ygy plugin marketplace add`.')
    } else {
      const names = km.marketplaces.map((m) => m.name).join(', ')
      console.error(
        `No cached marketplace index. You're subscribed to ${names} but the cache is empty — run \`ygy plugin marketplace refresh\` to fetch.`,
      )
    }
    return 1
  }
...
async function cliDoctor(): Promise<number> {
  ...
  console.log()
  console.log('For deeper diagnostics, set DEBUG_STDOUT=1 and check ~/.ygy-code/logs/debug.log')
...
async function cliMarketplace(args: string[]): Promise<number> {
  ...
  if (sub === 'add') {
    ...
    if (!name || !source) {
      console.error('Usage: ygy plugin marketplace add <name> <source>')
      return 1
    }
    try {
      await addKnownMarketplace({ name, source })
      console.log(chalk.green(`Subscribed to ${name} (${source})`))
      console.log(`Run 'ygy plugin marketplace refresh ${name}' to fetch its index.`)
      return 0
    } catch (err) {
      console.error(chalk.red(err instanceof Error ? err.message : String(err)))
      return 1
  ...
  if (sub === 'remove') {
    const name = rest[0]
    if (!name) {
      console.error('Usage: ygy plugin marketplace remove <name>')
      return 1
    }
  ...
  if (sub === 'info') {
    const name = rest[0]
    if (!name) {
      console.error('Usage: ygy plugin marketplace info <name>')
      return 1
    }
    const all = await readAllCachedMarketplaces()
    const m = all.find((x) => x.name === name)
    if (!m) {
      console.error(`No cached index for '${name}'. Run 'ygy plugin marketplace refresh ${name}' first.`)
      return 1
    }
  ...
  console.error('Usage: ygy plugin marketplace <list|add|remove|refresh|info> [args...]')
  return 1
}
