...
export function formatOpenAIAuthStatus(): string {
  ...
  if (status.mode === 'none') {
    return 'OpenAI authentication: none\nRun `/login` or `ygy login` for ChatGPT subscription access, or set OPENAI_API_KEY for Platform access.'
  }
...
export async function loginChatGPT(options: LoginChatGPTOptions = {}): Promise<LoginChatGPTResult> {
  ...
  const userAgent = `ygy-code/${VERSION}`
...
async function runLogin(deviceAuth: boolean, signal: AbortSignal): Promise<void> {
  ...
  if (result.apiKeyConfigured) console.log('OPENAI_API_KEY is now inactive for the OpenAI provider until `ygy logout`.')
}
  ...
  try {
    ...
    if (args[0] === 'login') {
      ...
      if (flags.some((flag) => flag !== '--device-auth')) {
        console.error('Usage: ygy login [--device-auth] | ygy login status')
        return 2
      }
    ...
    console.error('Usage: ygy login [--device-auth] | ygy login status | ygy logout')
    return 2
  } catch (err) {
