...
import { loadSession } from '@ygy-code/core'
  ...
  it('completes first-run setup and restores the parent shell on a clean exit', async () => {
    await withTui(
      ...
      async ({ harness, workspace }) => {
        expect(harness.text()).toContain('Welcome to ygy-code!')
        ...
        if (process.platform !== 'win32') {
          ...
          expect(shellOutput).toMatch(/__YGY_CODE_SHELL_OK_\d+__/)
        }
  ...
  it('forks completed context while the current request is still streaming', async () => {
    await withTui(
      ...
      async ({ harness, provider, workspace }) => {
        ...
        await harness.waitForText('ygy --resume "doc branch"')
        await submitInput(harness, '/fork')
        // Both forks race the stalled turn; wait for both branch files.
        const sessionDirectory = path.join(workspace.cwd, '.ygy-code', 'sessions')
        let files: string[] = []
        ...
        await harness.waitForText(`ygy --resume ${unnamed!.sessionId}`)
        expect(harness.raw()).toContain('ygy --resume "doc branch"')
        expect(harness.raw()).toContain(`ygy --resume ${unnamed!.sessionId}`)
