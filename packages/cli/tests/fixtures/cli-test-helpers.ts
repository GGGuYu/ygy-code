...
export async function createTestWorkspace(prefix = 'ygy-release-test-'): Promise<TestWorkspace> {
  const cwd = await fs.mkdtemp(path.join(os.tmpdir(), prefix))
  const xcodeHome = path.join(cwd, '.ygy-code-home')
...
export function isolatedCliEnv(
  ...
  Object.assign(env, {
    ...
    YGY_CODE_HOME: workspace.xcodeHome,
    NODE_ENV: 'test',
    NO_COLOR: '1',
    FORCE_COLOR: '0',
  ...
  if (provider) {
    ...
    env.YGY_CODE_MODEL = 'custom:test-model'
  }
  return env
}
...
export async function readSessionJsonl(cwd: string): Promise<unknown[]> {
  const dir = path.join(cwd, '.ygy-code', 'sessions')
