...
End-to-end tests that drive the real `ygy -p` binary against a real LLM.
...
| `ctx.runCli(prompt, opts?)`   | Spawn `ygy -p <prompt>` in tmpDir, return `RunResult`.            |
...
`<tmpDir>/.ygy-code/sessions/`, which encodes every assistant tool-call and
