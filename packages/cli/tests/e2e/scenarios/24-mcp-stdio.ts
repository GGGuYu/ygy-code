...
const scenario: Scenario = {
  ...
  async run(ctx) {
    ...
    //    YGY_CODE_HOME under <tmpDir>/.ygy-code-home, so the same scenario
    //    can run in parallel without trampling on the real ~/.ygy-code.
    await ctx.writeFile('mock-server.mjs', MOCK_SERVER_SRC)
    const serverPath = path.join(ctx.tmpDir, 'mock-server.mjs')

    await ctx.mkdir('.ygy-code-home')
    await ctx.writeFile(
      '.ygy-code-home/config.json',
      JSON.stringify(
        {
          // YGY_CODE_MODEL is set by the harness, so we don't need `model`.
          mcpServers: {
            mock: {
