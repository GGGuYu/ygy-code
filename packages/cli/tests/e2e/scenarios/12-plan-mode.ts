...
const scenario: Scenario = {
  ...
  async run(ctx) {
    ...
    // file under `.ygy-code/plans/`. enterPlanMode's tool result explicitly
    ...
    // tool touches anything outside `.ygy-code/plans/`.
    const isPlanFile = (p: unknown): boolean => typeof p === 'string' && /[\\/]\.ygy-code[\\/]plans[\\/]/.test(p)
    for (const tc of r.toolCalls) {
      if (tc.toolName !== 'writeFile' && tc.toolName !== 'edit') continue
      const filePath = tc.input.filePath
      ctx.expect.truthy(
        isPlanFile(filePath),
        `plan mode wrote outside .ygy-code/plans/: ${tc.toolName} → ${String(filePath)}`,
      )
    }
