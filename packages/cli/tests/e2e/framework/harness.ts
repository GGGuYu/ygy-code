// Harness: spawn `ygy -p` in print mode and parse the resulting session jsonl.
//
...
export async function runCliInDir(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    INIT_CWD: cwd,
    YGY_CODE_HOME: path.join(cwd, '.ygy-code-home'), // isolate user-scope ~/.ygy-code per scenario
    YGY_CODE_MODEL: cfg.modelId,
    NODE_ENV: 'test',
    NO_COLOR: '1',
    ...(options?.env ?? {}),
  }
  ...
  await waitForJsonlStable(path.join(cwd, '.ygy-code', 'sessions'))

  // Locate the freshest session jsonl in cwd/.ygy-code/sessions/
  const sessionJsonlPath = await pickLatestSessionJsonl(path.join(cwd, '.ygy-code', 'sessions'))
