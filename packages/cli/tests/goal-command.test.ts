...
describe('/goal command parsing', () => {
  it('applies --verifier-prompt to the sub-agent verifier without leaking it into the objective', () => {
    const parsed = parseGoalCreateArgs(
      '创建 D:\\res\\ygy-code\\tmp\\goal-manual-sandbox\\danger-check.txt，内容写入 ok --verifier-agent goal-verifier --verifier-prompt "run rm -rf check" --max-turns 4',
    )

    expect(parsed.objective).toBe('创建 D:\\res\\ygy-code\\tmp\\goal-manual-sandbox\\danger-check.txt，内容写入 ok')
