...
beforeAll(async () => {
  suiteRoot = await fs.mkdtemp(path.join(process.env.TMPDIR ?? process.env.TEMP ?? '/tmp', 'ygy-package-smoke-'))
  ...
  installedCliJs = path.join(installPrefix, 'node_modules', '@ygy-code', 'cli', 'dist', 'cli.js')
})
...
describe('published CLI tarball', () => {
  ...
  it('installs both bin aliases and runs version/help outside the repository', async () => {
    ...
    const ygy = path.join(binDir, `ygy${suffix}`)
    const xCode = path.join(binDir, `ygy-code${suffix}`)

    await expect(fs.access(ygy)).resolves.toBeUndefined()
    await expect(fs.access(xCode)).resolves.toBeUndefined()
    const version = await command(ygy, ['--version'], { cwd })
  ...
  it('runs a fake-provider print task from the installed package', async () => {
    ...
    const workspace = await createTestWorkspace('ygy-installed-task-')
    try {
  ...
  it('executes a tty shell tool through the bundled provider', async () => {
    ...
    const workspace = await createTestWorkspace('ygy-installed-pty-tool-')
    try {
      const result = await command(
        process.execPath,
  ...
  it('returns an actionable non-zero error in print mode when no API key is configured', async () => {
    const workspace = await createTestWorkspace('ygy-installed-no-key-')
    try {
  ...
  it('keeps first-run state inside the isolated project and YGY_CODE_HOME', async () => {
    const provider = await startFakeProvider([{ type: 'completion', text: 'first-run-ok' }])
    const workspace = await createTestWorkspace('ygy-first-run-中文 space-')
    try {
      ...
      expect(workspaceFiles.some((name) => name.startsWith('.ygy-code-home/'))).toBe(true)
      expect(workspaceFiles.some((name) => name.startsWith('.ygy-code/sessions/'))).toBe(true)
    } finally {
      await provider.close()
      await workspace.cleanup()
    }
