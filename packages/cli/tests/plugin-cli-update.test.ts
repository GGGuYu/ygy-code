...
//
// Each test runs in an isolated YGY_PLUGINS_DIR so installs land in a temp
// cache and don't bleed into the user's real ~/.ygy-code.
...
import { installPlugin, listInstalledPlugins } from '@ygy-code/core'
...
async function makeTempPlugin(body: Record<string, unknown>): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-update-src-'))
...
beforeEach(async () => {
  ...
  process.env.YGY_PLUGINS_DIR = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-update-cache-'))
...
describe('ygy plugin update — argv parsing', () => {
...
describe('ygy plugin update --all', () => {
