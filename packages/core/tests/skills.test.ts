...
async function makeTempSkillsDir(skills: { dir: string; frontmatter: string; body: string }[]): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-skills-test-'))
...
beforeEach(() => {
  originalSkillsDir = process.env.YGY_SKILLS_DIR
})

afterEach(async () => {
  if (originalSkillsDir === undefined) {
    delete process.env.YGY_SKILLS_DIR
  } else {
    process.env.YGY_SKILLS_DIR = originalSkillsDir
  }
})
...
describe('loadSkills', () => {
  it('returns empty array when directory does not exist', async () => {
    process.env.YGY_SKILLS_DIR = path.join(os.tmpdir(), 'ygy-skills-nonexistent-' + Date.now())
  ...
  it('loads a valid skill', async () => {
    ...
    process.env.YGY_SKILLS_DIR = dir
  ...
  it('skips SKILL.md with no frontmatter', async () => {
    const dir = path.join(os.tmpdir(), 'ygy-skills-nofm-' + Date.now())
...
describe('reloadSkillRegistry', () => {
  ...
  it('rescans disk + settings and mutates the registry in place', async () => {
    ...
    process.env.YGY_SKILLS_DIR = skillsDir
    const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-refresh-int-'))
...
describe('skill settings', () => {
  ...
  beforeEach(() => {
    originalHome = process.env.YGY_CODE_HOME
    originalCwd = process.cwd()
  })
  ...
  it('union of user + project disabled lists', async () => {
    ...
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-settings-test-home-'))
    const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-settings-test-proj-'))
    // utils.ts caches USER_YGY_DIR at module-eval time, so YGY_CODE_HOME
  ...
  it('setSkillDisabled returns noop when state already matches', async () => {
    ...
    const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-settings-noop-'))
    process.chdir(projectDir)
  ...
  it('preserves unrelated fields in settings.json', async () => {
    ...
    const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-settings-merge-'))
    process.chdir(projectDir)
...
describe('createSkillRegistry', () => {
  ...
  it('reads skills from disk, applies project-scope disable, and filters list/names/get', async () => {
    ...
    // Project-scope settings live under cwd/.ygy-code/settings.local.json.
    // Chdir to a fresh temp dir so we don't pollute the real repo or the
    // user's home (utils.ts caches USER_YGY_DIR at import time, so we
    // can't redirect user scope here — project scope is sufficient
    // because YGY_SKILLS_DIR also tags loaded skills as source='project').
    const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-registry-int-'))
    process.chdir(projectDir)
