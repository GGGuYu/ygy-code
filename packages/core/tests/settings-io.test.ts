...
describe('settings file mutations', () => {
  it('does not create or rewrite a file for a no-op mutation', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-settings-io-'))
    ...
    const pluginFile = path.join(root, '.ygy-code', 'settings.local.json')
  ...
  it('preserves unrelated settings while updating and clearing plugin state', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-settings-plugin-'))
    const file = path.join(root, '.ygy-code', 'settings.local.json')
