...
beforeEach(async () => {
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugins-int-test-'))
...
describe('buildPluginIntegration', () => {
  it('lists skill + agent dirs for each enabled plugin (resolved absolute)', async () => {
    ...
    const src = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-int-plugin-src-'))
    await writeFileAt(
    ...
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-int-cwd-'))
  ...
  it('records hook config parse errors per plugin without crashing the rest', async () => {
    const bad = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-int-plugin-bad-'))
    await writeFileAt(
  ...
  it("accepts a flat `.mcp.json` (no `mcpServers` wrapper) — Claude Code's official plugin shape", async () => {
    ...
    const src = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-int-plugin-flat-mcp-'))
  ...
  it('resolves mcpServers name collisions first-wins + records the loser', async () => {
    ...
    const srcA = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-int-plugin-srcA-'))
    await writeFileAt(
    ...
    const srcB = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-int-plugin-srcB-'))
    await writeFileAt(
  ...
  it('records mcp parse errors per plugin without killing the others', async () => {
    ...
    const good = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-int-plugin-good-'))
    await writeFileAt(
