...
describe('expandVariables', () => {
  ...
  it('reads ${env:NAME} from process.env', () => {
    process.env.YGY_HOOK_TEST_VAR = 'hello'
    try {
      expect(expandVariables('echo ${env:YGY_HOOK_TEST_VAR}', ctx)).toBe('echo hello')
    } finally {
      delete process.env.YGY_HOOK_TEST_VAR
    }
  })
  ...
  it('${pluginDataDir} expands and auto-creates the dir when pluginId is supplied', async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-data-ctx-'))
...
describe('executeHook (real subprocess)', () => {
  async function writeHookScript(contents: string): Promise<string> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-hook-script-'))
