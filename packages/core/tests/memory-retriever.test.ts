...
describe('MemoryRetriever', () => {
  const product = parseMemoryTopic(
    topicMarkdown({
      ...
      facts: [{ id: 'portfolio.ygy-code.stack', content: '- ygy-code uses TypeScript.' }],
    }),
  ...
  it('directly selects one protected exact alias and applies repository boost', () => {
    const result = retriever.retrieve(query('ygy-code'))
  ...
  it('packs only selected topic sections within the token budget', () => {
    const attachment = retriever.pack(query('ygy-code TypeScript'), ['product-portfolio'], 4)
