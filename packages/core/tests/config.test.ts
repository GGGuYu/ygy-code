...
describe('context window config', () => {
  ...
  beforeEach(() => {
    isolateUserConfig()
    home = process.env.YGY_CODE_HOME!
  })
 ...
 *  ~/.ygy-code/config.json (possibly written by a recent /model switch)
 *  can't contaminate these assertions. */
function isolateUserConfig(): void {
  const tmp = path.join(os.tmpdir(), 'ygy-code-config-test-' + Math.random().toString(36).slice(2))
  process.env.YGY_CODE_HOME = tmp
}

describe('resolveModelId', () => {
  beforeEach(() => {
    isolateUserConfig()
    delete process.env.YGY_CODE_MODEL
  ...
  it('falls back to env var YGY_CODE_MODEL', () => {
    process.env.YGY_CODE_MODEL = 'openai:gpt-5.6-sol'
    expect(resolveModelId()).toBe('openai:gpt-5.6-sol')
  })

  it('resolves alias from YGY_CODE_MODEL env var', () => {
    process.env.YGY_CODE_MODEL = 'sonnet'
    expect(resolveModelId()).toBe('anthropic:claude-sonnet-5')
  })

  it('CLI argument takes precedence over YGY_CODE_MODEL', () => {
    process.env.YGY_CODE_MODEL = 'openai:gpt-5.6-sol'
    expect(resolveModelId('sonnet')).toBe('anthropic:claude-sonnet-5')
  })
