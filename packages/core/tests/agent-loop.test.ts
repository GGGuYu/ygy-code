...
// project's `.ygy-code/sessions/` (which would leak between runs and pollute
...
describe('agent loop', () => {
  ...
  it('enqueues exactly one durable memory job after a clean root stop and never for a sub-agent', async () => {
    ...
    await agentLoop(
      'Remember that ygy-code is my product',
      {} as any,
  ...
  it('classifies managed-memory mutations while preserving read-only diagnostics', () => {
    const memoryRoot = path.join(os.homedir(), '.ygy-code', 'memory')
    ...
    expect(isManagedMemoryMutation('shell', { command: 'ls ~/.ygy-code/memory/topics 2>/dev/null' }, memoryRoot)).toBe(
      false,
    )
    expect(
      isManagedMemoryMutation(
        'shell',
        { command: "sed -i 's/alpha/beta/' ~/.ygy-code/memory/topics/profile.md" },
        memoryRoot,
      ),
    ).toBe(true)
    expect(
      isManagedMemoryMutation('shell', { command: 'echo beta > ~/.ygy-code/memory/topics/profile.md' }, memoryRoot),
    ).toBe(true)
  })
