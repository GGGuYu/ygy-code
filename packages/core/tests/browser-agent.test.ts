...
describe('buildBrowserServerConfig', () => {
  ...
  it('redirects saved output off the repo via --output-dir (npx path only)', async () => {
    ...
    expect(path.basename(outputDirectory)).toMatch(/^ygy-code-browser-/)
...
describe('browser sub-agent registration gating', () => {
  ...
  beforeAll(async () => {
    // Isolate from the developer's real ~/.ygy-code so the custom-agent disk scan
    // and the default config gate both see an empty home.
    prevHome = process.env.YGY_CODE_HOME
    home = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-browser-'))
    process.env.YGY_CODE_HOME = home
  })
  afterAll(async () => {
