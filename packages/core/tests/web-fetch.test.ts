...
describe('webFetch redirect validation', () => {
  it('blocks a public URL that redirects to a private address', async () => {
    ...
    await expect(doFetch('https://example.com/start', 'ygy-code/test', undefined, mockFetch)).rejects.toThrow(
  ...
  it('propagates cancellation to the active request', async () => {
    ...
    const pending = doFetch('https://example.com/slow', 'ygy-code/test', controller.signal, mockFetch)
    controller.abort()
