...
async function makeTempPluginDir(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugin-test-'))
}
...
describe('discoverManifest', () => {
  ...
  it('finds .ygy-code-plugin/plugin.json (native, highest priority)', async () => {
    const root = await makeTempPluginDir()
    const expected = await writeManifest(root, '.ygy-code-plugin/plugin.json', { name: 'foo', version: '1.0.0' })
