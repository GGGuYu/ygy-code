...
describe('webSearch', () => {
  ...
  it('honors the YGY_CODE_WEB_SEARCH_PROVIDER override', async () => {
    clearSearchEnv()
    vi.stubEnv('TAVILY_API_KEY', 'tavily-key')
    vi.stubEnv('EXA_API_KEY', 'exa-key')
    vi.stubEnv('YGY_CODE_WEB_SEARCH_PROVIDER', 'exa')
    const mockFetch = stubFetchJson({ results: [] })
  ...
  it('reports an unknown override provider', async () => {
    clearSearchEnv()
    vi.stubEnv('YGY_CODE_WEB_SEARCH_PROVIDER', 'nope')
