...
describe('glob tool', () => {
  it('finds files matching a pattern', async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-glob-test-'))
  ...
  it('truncates results when exceeding the cap (200)', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-glob-cap-'))
    const count = 210
  ...
  it('returns files sorted by modification time, most recent first', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-glob-sort-'))
  ...
  it('catch-all pattern (**/*) honors .gitignore', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-glob-catchall-'))
  ...
  it('excludes .git/ directory contents', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-glob-gitdir-'))
