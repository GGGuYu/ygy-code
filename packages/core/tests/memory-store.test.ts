...
describe('MemoryStore', () => {
  it('round-trips frontmatter, fact markers, and manual text', async () => {
    ...
    const raw = topicMarkdown({
      ...
      facts: [{ id: 'portfolio.ygy-code.stack', content: '- Stack is TypeScript.' }],
    })
    const parsed = parseMemoryTopic(raw, filePath)
    const again = parseMemoryTopic(formatMemoryTopic(parsed), filePath)
    expect(again.facts[0]?.metadata.id).toBe('portfolio.ygy-code.stack')
  ...
  it('physically replaces a conflicting fact and removes every old value', async () => {
    ...
    await writeTopic(
      ...
      topicMarkdown({
        ...
        aliases: ['ygy-code'],
        keywords: ['stack'],
        facts: [{ id: 'portfolio.ygy-code.stack', content: '- Stack is JavaScript.' }],
      }),
      'product',
    )
    ...
    const result = await store.applyOperations([
      {
        action: 'upsert',
        topicId: 'product',
        factId: 'portfolio.ygy-code.stack',
    ...
    expect(written.match(/portfolio\.ygy-code\.stack/g)).toHaveLength(1)
  ...
  it('reuses an existing fact ID when the structural subject and predicate match', async () => {
    ...
    expect(reloaded.facts.map((fact) => fact.metadata.id)).toEqual(['portfolio.ygy-code.stack'])
  ...
  it('moves a replaced fact across topics and deletes the empty old topic', async () => {
    ...
    await writeTopic(
      ...
      topicMarkdown({
        id: 'old-product',
        type: 'portfolio',
        facts: [{ id: 'portfolio.ygy-code.owner', content: '- ygy-code belongs to the old owner.' }],
      }),
      'old-product',
    )
    ...
    const result = await store.applyOperations([
      {
        ...
        factId: 'portfolio.ygy-code.owner',
    ...
    expect(replacement).toContain('The user owns ygy-code')
