...
async function makeTempPlugin(body: Record<string, unknown>, rel = 'plugin.json'): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugin-src-'))
...
beforeEach(async () => {
  originalPluginsDir = process.env.YGY_PLUGINS_DIR
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugins-cache-test-'))
  process.env.YGY_PLUGINS_DIR = tmp
})
...
describe('installPlugin (local source)', () => {
  ...
  it('rejects a source with no manifest', async () => {
    const src = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugin-empty-'))
  ...
  it('also copies non-manifest files (skills/, agents/, etc.) into the cache', async () => {
    const src = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugin-rich-'))
    await fs.writeFile(
  ...
  it.skipIf(process.platform === 'win32')('drops symlinks that escape the plugin source tree', async () => {
    ...
    const outside = path.join(await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugin-outside-')), 'secret.txt')
...
describe('install policy gates (strictKnownMarketplaces + blockedPlugins)', () => {
  ...
  it('strictKnownMarketplaces rejects installs from a non-subscribed marketplace', async () => {
    const file = path.join(process.env.YGY_PLUGINS_DIR!, 'known_marketplaces.json')
    await fs.mkdir(path.dirname(file), { recursive: true })
...
describe('loadAllPlugins', () => {
  it('returns empty registry when disabled', async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-loader-cwd-'))
  ...
  it('loads project-local plugins from <cwd>/.ygy-code/plugins/<name>/', async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-loader-cwd-'))
    const pluginDir = path.join(cwd, '.ygy-code', 'plugins', 'inhouse')
  ...
  it('records load errors for broken manifests without aborting', async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-loader-cwd-'))
    const broken = path.join(cwd, '.ygy-code', 'plugins', 'broken')
    await fs.mkdir(broken, { recursive: true })
    await fs.writeFile(path.join(broken, 'plugin.json'), '{ not json', 'utf-8')

    const good = path.join(cwd, '.ygy-code', 'plugins', 'good')
  ...
  it('records a Gemini-extension load error without crashing', async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-loader-cwd-'))
    const gemini = path.join(cwd, '.ygy-code', 'plugins', 'gemini-plugin')
  ...
  it('respects enable flags from <cwd>/.ygy-code/settings.local.json', async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-loader-cwd-'))
    const pluginDir = path.join(cwd, '.ygy-code', 'plugins', 'disabled-one')
    await fs.mkdir(pluginDir, { recursive: true })
    await fs.writeFile(path.join(pluginDir, 'plugin.json'), JSON.stringify({ name: 'disabled-one', version: '1.0.0' }))

    await fs.writeFile(
      path.join(cwd, '.ygy-code', 'settings.local.json'),
...
describe('resolveContributions', () => {
  ...
  it('auto-discovers commands/ agents/ skills/ when manifest omits them (Claude Code convention)', async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-resolve-conv-'))
...
async function makeLocalGitRepo(manifest: Record<string, unknown>): Promise<{ url: string; sha: string }> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-git-src-'))
