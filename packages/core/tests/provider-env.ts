...
/** Keep provider-policy tests independent from a developer's real `ygy login`. */
export function isolateOpenAIAuth(): () => void {
  const previousHome = process.env.YGY_CODE_HOME
  const isolatedHome = path.join(os.tmpdir(), `ygy-code-provider-auth-${crypto.randomUUID()}`)
  ...
  return () => {
    resetOpenAIAuthContextForTesting()
    resetOpenAIChatGPTModelsForTesting()
    if (previousHome === undefined) delete process.env.YGY_CODE_HOME
    else process.env.YGY_CODE_HOME = previousHome
