...
beforeEach(async () => {
  testDir = await mkdtemp(path.join(tmpdir(), 'ygy-code-registry-'))
  process.env.YGY_CODE_HOME = path.join(testDir, 'nested', 'ygy-code-home')
})
...
describe('owner-only peer registry', () => {
  itPosix('rejects a symlink YGY_CODE_HOME without changing its target permissions', async () => {
    const target = path.join(testDir, 'target-home')
    await mkdir(target, { mode: 0o755 })
    const linked = path.join(testDir, 'linked-home')
    await symlink(target, linked)
    process.env.YGY_CODE_HOME = linked
  ...
  it('routes through YGY_CODE_HOME and atomically writes owner-only registrations', async () => {
    const registry = createPeerRegistry()
    await registry.initialize()
    const paths = registry.paths()
    expect(paths.registryDir).toBe(path.join(process.env.YGY_CODE_HOME!, 'runtime', 'peers'))
