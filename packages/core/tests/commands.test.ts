...
/** loadPluginCommands now also scans `${YGY_CODE_HOME}/commands` and
 *  `<cwd>/.ygy-code/commands` — isolate every test under fresh empty dirs so
 *  the developer's real `~/.ygy-code/commands/` doesn't leak into assertions. */
...
beforeEach(async () => {
  prevHome = process.env.YGY_CODE_HOME
  prevCwd = process.cwd()
  isolatedHome = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-cmds-home-'))
  isolatedCwd = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-cmds-cwd-'))
  process.env.YGY_CODE_HOME = isolatedHome
  process.chdir(isolatedCwd)
})

afterEach(async () => {
  if (prevHome === undefined) delete process.env.YGY_CODE_HOME
  else process.env.YGY_CODE_HOME = prevHome
...
describe('loadPluginCommands', () => {
  it('loads each commands/<name>.md as a CommandDefinition with name = basename', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-cmds-'))
  ...
  it('handles real Claude Code multi-line allowed-tools frontmatter without crashing', async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-cmds-allowed-'))
    await writeCommand(
      dir,
      'real-shape',
  ...
  it('treats a file with no frontmatter as a body-only command (no description)', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-cmds-nofm-'))
  ...
  it('scans user (~/.ygy-code/commands) and project (<cwd>/.ygy-code/commands) directories', async () => {
    await writeCommand(path.join(isolatedHome, 'commands'), 'usercmd', 'description: from user', 'user body')
    await writeCommand(
      path.join(isolatedCwd, '.ygy-code', 'commands'),
      'projcmd',
  ...
  it('precedence: project overrides plugin overrides user on name conflict', async () => {
    ...
    const pluginDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-cmds-plugin-'))
    await writeCommand(pluginDir, 'dup', 'description: plugin', 'plugin-body')
    await writeCommand(path.join(isolatedCwd, '.ygy-code', 'commands'), 'dup', 'description: project', 'project-body')
...
describe('expandCommandBody', () => {
  ...
  it('substitutes ${CLAUDE_PLUGIN_DATA} with the plugin data dir and auto-creates it', async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-cmd-data-'))
    const prev = process.env.YGY_PLUGINS_DIR
    process.env.YGY_PLUGINS_DIR = tmpHome
    try {
      ...
      if (prev === undefined) delete process.env.YGY_PLUGINS_DIR
      else process.env.YGY_PLUGINS_DIR = prev
