...
describe('tokenize', () => {
  ...
  it('preserves backslash-non-special as a literal pair (Windows paths)', () => {
    ...
    // ate path separators on Windows. `D:\res\ygy-code\tmp` MUST survive.
    expect(tokenize('D:\\res\\ygy-code\\tmp')).toEqual({
      ok: true,
      tokens: ['D:\\res\\ygy-code\\tmp'],
    })
  })
...
describe('parseAdd — stdio', () => {
  ...
  it('preserves Windows-style backslash paths in args', () => {
    // Regression for the bug where `D:\res\ygy-code\tmp` got mangled
    // into `D:resygy-codetmp` because the tokenizer ate the backslashes.
    const r = parseAdd('fs npx -y @modelcontextprotocol/server-filesystem D:\\res\\ygy-code\\tmp')
    expect(r.ok).toBe(true)
    if (r.ok) {
      const cfg = r.command.config as { args: string[] }
      expect(cfg.args).toEqual(['-y', '@modelcontextprotocol/server-filesystem', 'D:\\res\\ygy-code\\tmp'])
    }
  })
