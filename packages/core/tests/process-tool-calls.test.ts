...
describe('processToolCalls plan-mode boundary', () => {
  ...
  it('rejects writes outside the current plan file without prompting', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-plan-boundary-'))
  ...
  it('writes the current plan file without a second permission dialog', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-plan-write-'))
    temporaryDirectories.push(dir)
    const planPath = path.join(dir, '.ygy-code', 'plans', 'plan.md')
    const state = createLoopState('plan', { projectCwd: dir })
    state.currentPlanPath = planPath
  ...
  it('rejects a plan path whose plans directory resolves outside the project', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-plan-link-'))
    temporaryDirectories.push(dir)
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-plan-outside-'))
    temporaryDirectories.push(outside)
    const planRoot = path.join(dir, '.ygy-code', 'plans')
  ...
  it('rejects shell calls even when trust mode is enabled', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-plan-shell-'))
    temporaryDirectories.push(dir)
    const state = createLoopState('plan', { projectCwd: dir })
...
describe('processToolCalls relative write paths', () => {
  it('anchors a relative writeFile path at projectCwd, not process.cwd()', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-rel-write-'))
    temporaryDirectories.push(dir)
...
describe('processToolCalls edit validation', () => {
  async function runInvalidEdit(oldString: string, newString: string) {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-edit-validation-'))
    temporaryDirectories.push(dir)
  ...
  it('normalizes JSON batch input before the loop guard and applies one atomic write', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-batch-edit-'))
    temporaryDirectories.push(dir)
    const filePath = path.join(dir, 'input.txt')
  ...
  it('leaves the file unchanged when any batch replacement fails', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-batch-edit-failure-'))
    temporaryDirectories.push(dir)
    const filePath = path.join(dir, 'input.txt')
...
describe('processToolCalls visual-check retry policy', () => {
  ...
  it('resets the visual-check budget after a successful file edit', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-visual-budget-'))
    temporaryDirectories.push(dir)
...
describe('processToolCalls rewind origin capture', () => {
  it('restores an existing file first modified by writeFile after the checkpoint', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-write-rewind-'))
    temporaryDirectories.push(dir)
...
describe('processToolCalls skip-fulfilled (SDK already produced a tool-result)', () => {
  it('keeps read-only managed-memory shell diagnostics out of UI callbacks', async () => {
    const memoryRoot = path.join(os.tmpdir(), 'ygy-code-managed-memory')
