...
beforeEach(() => {
  ...
  // dev's actual repo (`.ygy-code/sessions/` is at process.cwd()).
  tempDir = mkdtempSync(join(tmpdir(), 'ygy-session-store-'))
...
describe('session-store: filename derivation', () => {
  ...
  it('preserves a pinned legacy slug-prefixed path', () => {
    const legacyPath = join(tempDir, '.ygy-code', 'sessions', 'fix-login-20260101-120000-000.jsonl')
    const state = {
...
describe('session-store: malformed input', () => {
  ...
  it('returns null when the file has no parseable header', async () => {
    const sessionsDir = join(tempDir, '.ygy-code', 'sessions')
