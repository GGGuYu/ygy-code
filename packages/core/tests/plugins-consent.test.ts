...
beforeEach(async () => {
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugin-consent-test-'))
...
describe('buildConsentPreview', () => {
  ...
  it('surfaces auto-discovered root .mcp.json (flat shape) via rootProbe', async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-consent-probe-flat-'))
    await fs.writeFile(
  ...
  it('surfaces auto-discovered root .mcp.json (wrapped shape) via rootProbe', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-consent-probe-wrapped-'))
  ...
  it('surfaces auto-discovered skills/agents/commands dirs via rootProbe', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-consent-probe-dirs-'))
...
describe('installPlugin + consent', () => {
  async function makeLocalPlugin(): Promise<string> {
    const src = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-consent-src-'))
    await writeFileAt(
