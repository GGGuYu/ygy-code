...
describe('getPermissionLevel', () => {
  ...
  it('auto-allows real PowerShell read-only pipelines', () => {
    expect(
      getPermissionLevel('shell', {
        command:
          "cd D:\\res\\ygy-code\\packages\\core\\src; Get-ChildItem -Recurse -Filter *.ts | Group-Object Directory | Sort-Object Name | Select-Object @{N='Directory';E={$_.Name}},Count",
      }),
    ).toBe('always-allow')
    expect(
      getPermissionLevel('shell', {
        command:
          "Get-ChildItem -Recurse -Filter *.ts -Path D:\\res\\ygy-code\\packages\\core\\src | Group-Object Directory | Sort-Object Name | Select-Object @{Name='Directory';Expression={$_.Name}},Count",
      }),
    ).toBe('always-allow')
  })
  ...
  it('auto-allows `ls X; if (Test-Path X) { Get-Content X }` end-to-end', () => {
    ...
    expect(
      getPermissionLevel('shell', {
        command:
          'ls -Force D:\\res\\ygy-code\\.ygy-code\\local\\permissions.json 2>&1; if (Test-Path D:\\res\\ygy-code\\.ygy-code\\local\\permissions.json) { Get-Content D:\\res\\ygy-code\\.ygy-code\\local\\permissions.json }',
      }),
    ).toBe('always-allow')
  })
})
...
describe('extractCommandPrefix', () => {
  ...
  it('recognises bare PowerShell cmdlets as their own prefix', () => {
    ...
    expect(extractCommandPrefix('Get-ChildItem -Recurse -Filter *.ts -Path D:\\res\\ygy-code')).toBe('Get-ChildItem')
...
// and in what ends up in .ygy-code/local/permissions.json. Indirect
...
describe('isPathWithinProject', () => {
  ...
  it('returns false when an in-project directory link resolves outside', async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-path-boundary-'))
    try {
