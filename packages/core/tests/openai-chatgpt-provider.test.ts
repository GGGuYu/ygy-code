...
describe('OpenAI ChatGPT provider fetch', () => {
  ...
  beforeEach(async () => {
    testHome = path.join(os.tmpdir(), `ygy-code-chatgpt-provider-${crypto.randomUUID()}`)
  ...
  it('rewrites only to the ChatGPT backend and never sends OPENAI_API_KEY', async () => {
    ...
    expect(headers.get('originator')).toBe('ygy-code')
  ...
  it('makes subscription usage limits non-retryable while preserving quota headers', async () => {
    const transport = vi.fn<typeof fetch>(async () =>
      Response.json(
        { error: { type: 'usage_limit_reached', message: 'usage_limit_reached' } },
        {
          ...
          headers: {
            'retry-after': '60',
            'ygy-codex-primary-used-percent': '100.0',
            'ygy-codex-primary-reset-at': '2000000000',
          },
        },
      ),
    )
    ...
    expect(response.headers.get('ygy-codex-primary-used-percent')).toBe('100.0')
  })
  ...
  it('requires a new login when OAuth refresh is permanently invalid even if OPENAI_API_KEY exists', async () => {
    ...
    ).rejects.toMatchObject({ code: 'login-required', message: expect.stringContaining('ygy login') })
    expect(transport).not.toHaveBeenCalled()
