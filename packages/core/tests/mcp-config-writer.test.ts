...
/** Each test gets its own scratch ~/.ygy-code under tmpdir, plus a scratch
...
describe('config-writer: project scope', () => {
  ...
  it('writes to <project>/.ygy-code/config.json', async () => {
    const res = await writeServerToConfig('foo', { command: 'node', args: ['f.js'] }, 'project', ctx.project)
    expect(res.path).toContain(path.join('.ygy-code', 'config.json'))
