...
/** Each test gets its own scratch ~/.ygy-code under tmpdir so we never touch
...
describe('promptForTrust', () => {
  ...
  it('maps "Exit ygy-code" answer to "exit"', async () => {
    const choice = await promptForTrust('/p', [{ name: 's', preview: 'cmd' }], async () => 'Exit ygy-code')
