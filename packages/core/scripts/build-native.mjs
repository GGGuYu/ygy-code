...
async function main() {
  ...
  const source = path.join(nativeDir, 'target', 'release', 'ygy-shell-supervisor.exe')
  const destinationDir = path.join(coreDir, 'dist', 'native', 'windows', process.arch)
  const destination = path.join(destinationDir, 'ygy-shell-supervisor.exe')
  ...
  manifest.artifacts[process.arch] = {
    file: `${process.arch}/ygy-shell-supervisor.exe`,
    sha256,
  }
