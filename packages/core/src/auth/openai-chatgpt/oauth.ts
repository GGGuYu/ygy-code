...
export async function loginOpenAIChatGPTWithBrowser(
  ...
  const server = createServer((request, response) => {
    ...
    if (settled || callbackAccepted) {
      ...
      response.end(callbackPage('ChatGPT login already handled', 'Return to ygy-code to continue.'))
      return
    }
      ...
      try {
        ...
        if (response.destroyed) {
          ...
          try {
            ...
            response.once('close', finish)
            response.end(
              callbackPage('ChatGPT login complete', 'You can close this window and return to ygy-code.'),
              finish,
            )
          } catch {
            finish()
        ...
        if (!response.destroyed && !response.writableEnded) {
          ...
          response.once('close', finish)
          response.end(
            callbackPage('ChatGPT login failed', 'Return to ygy-code for details, then try again.'),
            finish,
          )
          return
        }
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === 'EADDRINUSE') {
      throw new OpenAIChatGPTAuthError(
        'oauth-failed',
        'Ports 1455 and 1457 are already in use. Close another login process or run `ygy login --device-auth`.',
        { cause: err },
      )
    }
