// @ygy-code/core — Shared utilities and constants
...
export const YGY_DIR = '.ygy-code'

/** User-scope config directory (~/.ygy-code). Frozen at module load — use
 *  {@link userYgyDir} when you want the `YGY_CODE_HOME` override applied. */
export const USER_YGY_DIR = path.join(os.homedir(), '.ygy-code')

/** Resolve the user-scope config root at CALL TIME, honouring `YGY_CODE_HOME`.
 *  Use this in path helpers across the codebase so a single env var can
 *  reroute everything that lives under `~/.ygy-code/` for sandbox testing
 *  or per-user isolation. Falling back to the frozen {@link USER_YGY_DIR}
 *  keeps the normal case allocation-free. */
export function userYgyDir(): string {
  return process.env.YGY_CODE_HOME ?? USER_YGY_DIR
}

// ── Debug log (shared by core + cli) ────────────────────────────────────
// Turn on with `DEBUG_STDOUT=1`. Writes to ~/.ygy-code/logs/debug.log so a
...
const LOG_DIR = path.join(USER_YGY_DIR, 'logs')
 ...
 *  `YGY_PLUGIN_DEBUG=1`), debugLog also mirrors lines tagged with one of
