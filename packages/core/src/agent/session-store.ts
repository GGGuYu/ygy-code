// @ygy-code/core — Per-session JSONL transcript store.
//
// One file per session: `.ygy-code/sessions/<sessionId>.jsonl`.
...
import { YGY_DIR, generateTimestampId } from '../utils.js'
...
function sessionsDir(cwd: string = process.cwd()): string {
  return path.join(cwd, YGY_DIR, SESSIONS_SUBDIR)
}
 ...
 *  under `.ygy-code/file-history/<sessionId>/`. */
...
export interface LoadedSession {
   ...
   *  The backing file manifests live under `.ygy-code/file-history/<sid>/`. */
 ...
 *  null if none exist. Used by `ygy --continue` / `-c` to skip the picker
