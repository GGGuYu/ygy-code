// @ygy-code/core — File-history snapshot store (content-addressed)
//
...
// occupy disk once. Per session, layout under .ygy-code/file-history/<sid>/:
//
...
import { YGY_DIR, fileExists, generateTimestampId } from '../utils.js'
...
function historyDir(sessionId: string, cwd: string): string {
  return path.join(cwd, YGY_DIR, FILE_HISTORY_SUBDIR, sessionId)
}
