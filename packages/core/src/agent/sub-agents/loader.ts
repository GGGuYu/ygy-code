// @ygy-code/core — Custom sub-agent loader
//
// Scans ~/.ygy-code/agents/*.md and <repo-root>/.ygy-code/agents/*.md for
 ...
 *  extra dirs (plugin-contributed). Environment variable `YGY_AGENTS_DIR`
...
export async function loadCustomAgents(opts: LoadCustomAgentsOptions = {}): Promise<SubAgentDefinition[]> {
  const override = process.env.YGY_AGENTS_DIR
  if (override) {
  ...
  const userDir = path.join(userYgyDir(), 'agents')
  const projectDir = path.join(process.cwd(), YGY_DIR, 'agents')
