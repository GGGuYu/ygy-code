// @ygy-code/core — Plan-mode file storage
//
// Plans live in `.ygy-code/plans/<slug>-<YYYYMMDD-HHMMSS>.md` inside the
// user's project (NOT in the user-scope `~/.ygy-code/`). This mirrors how
// `.ygy-code/sessions/` and `.ygy-code/memory/` are scoped: per-project,
// gitignored, never shared across repos. The slug-then-timestamp shape
// matches the legacy filenames already living under `.ygy-code/plans/`
...
function plansDir(): string {
  return path.join(process.cwd(), YGY_DIR, PLANS_SUBDIR)
}
 ...
 *  to also ensure `.ygy-code/` separately — first plan written in a fresh
