// @ygy-code/core — Tool execution & dispatch
...
import { YGY_DIR, debugLog, isAbortError } from '../utils.js'
 ...
 *  ygy-code what to do instead" their typed feedback rides along so the
...
function enforcePlanModeBoundary(ctx: HandlerCtx): boolean {
  ...
  const planRoot = path.join(ctx.state.projectCwd, YGY_DIR, 'plans')
  if (
    !target ||
    !planPath ||
