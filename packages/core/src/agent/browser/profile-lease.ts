// @ygy-code/core — Cross-process ownership for Playwright's persistent
...
// ygy process fail early with a useful message while retaining login state.
...
const PROFILE_LOCK_FILE = '.ygy-code-profile.lock'
...
export async function acquireBrowserProfileLease(
  ...
  if (!lease) {
    throw new Error(
      'The managed browser profile for this workspace is already in use by another ygy process. ' +
        'Close the other ygy session and retry; ygy-code will not replace the persistent signed-in profile with a temporary one.',
    )
  }
  return lease
}
