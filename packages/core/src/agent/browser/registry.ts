// @ygy-code/core — Managed browser MCP bootstrap
//
...
const TAB_CLEANUP_MARKER = '__YGY_CODE_VISUAL_TAB_CLEANUP__:'
const TAB_PREPARED_MARKER = '__YGY_CODE_VISUAL_TAB_PREPARED__:'
const TAB_OWNED_MARKER = '__YGY_CODE_VISUAL_TAB_OWNED__:'
...
function browserOutputDirectory(): string {
  if (!managedOutputDirectory) {
    ...
    managedOutputDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'ygy-code-browser-'))
  } else {
  ...
  try {
    ...
    const value: BrowserMcp = {
      ...
      prepareVisualCheck: async (ownerId, signal) => {
        ...
        const originalKey = `ygy-code-visual-original:${ownerId}`
      ...
      markVisualCheckTab: async (ownerId, url, signal) => {
        if (!result.tools.some((tool) => tool.name === PRIVATE_BROWSER_CODE_TOOL)) return false
        const originalKey = `ygy-code-visual-original:${ownerId}`
        const temporaryKey = `ygy-code-visual-temporary:${ownerId}`
