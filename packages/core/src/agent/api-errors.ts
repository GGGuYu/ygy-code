// @ygy-code/core — API error classification & pattern detection
import {
...
function chatGPTUsageLimitMessage(err: unknown): string {
  const usedPercent = extractResponseHeader(err, 'ygy-codex-primary-used-percent')
  const resetAt = Number(extractResponseHeader(err, 'ygy-codex-primary-reset-at'))
...
export function classifyApiError(err: unknown): ClassifiedError {
  ...
  if (isChatGPTLoginRequiredError(err)) {
    return {
      message:
        'ChatGPT sign-in expired or was revoked. Run `ygy login` again, or `ygy logout` to return to OpenAI API key authentication.',
      retryable: false,
    }
  }
  if (isUnauthorizedError(msg, status)) {
    return {
      message: chatGPTAuth
        ? 'ChatGPT authentication failed (401). Run `ygy login` again, or `ygy logout` to return to OpenAI API key authentication.'
        : 'API authentication failed (401). Please check your API key with /model or reconfigure with `ygy init`.',
      retryable: false,
    }
  }
