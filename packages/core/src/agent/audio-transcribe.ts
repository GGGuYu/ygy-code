// @ygy-code/core — Local audio transcription via whisper.cpp
//
...
// first use and cached under ~/.ygy-code/whisper-models/.
//
// Platform support:
...
import { debugLog, errorMessage, userYgyDir } from '../utils.js'
...
const DEFAULT_MODEL: WhisperModelName = (process.env.YGY_CODE_WHISPER_MODEL as WhisperModelName | undefined) ?? 'tiny'
...
function modelsDir(): string {
  return path.join(userYgyDir(), 'whisper-models')
}
...
async function ensureModel(model: WhisperModelName, options?: EnsureModelOptions): Promise<string> {
  ...
  try {
    ...
    const manualHint =
      ...
      `Or set YGY_CODE_WHISPER_MODEL=tiny.en for an English-only model.`
...
async function stageAudioFile(
  ...
  abortSignal?.throwIfAborted()
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-code-audio-'))
