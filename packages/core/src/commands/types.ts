// @ygy-code/core — File-based slash command types
//
// A "command" is a markdown file shipped by a plugin (or, in future,
// authored directly under ~/.ygy-code/commands/) that turns into a slash
...
export interface CommandDefinition {
  ...
  body: string
  /** Where this command came from. `'user'` = `~/.ygy-code/commands/*.md`,
   *  `'project'` = `<repo-root>/.ygy-code/commands/*.md`, `'plugin'` =
