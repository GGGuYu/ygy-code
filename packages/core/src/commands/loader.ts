// @ygy-code/core — File-based command loader
//
...
/** Load slash commands from user (`~/.ygy-code/commands/*.md`) + plugin
 *  (extraDirs) + project (`<cwd>/.ygy-code/commands/*.md`) sources.
 *  Merge order is user → plugin → project so CommandRegistry's
 *  last-write-wins yields the precedence **project > plugin > user** —
 *  same as skills + sub-agents. `userYgyDir()` is called at load time so
 *  `YGY_CODE_HOME` (used by tests) redirects the user-scope path. */
export async function loadPluginCommands(opts: LoadCommandsOptions = {}): Promise<CommandDefinition[]> {
  const userDir = path.join(userYgyDir(), 'commands')
  const projectDir = path.join(process.cwd(), YGY_DIR, 'commands')
