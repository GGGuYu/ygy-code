// @ygy-code/core — Knowledge loader
//
...
export async function buildKnowledgeContext(options?: {
  ...
  // CLAUDE.md so users with an existing `~/.ygy-code/CLAUDE.md` (or one
  // copied over from Claude Code's home) get it picked up without
  // having to rename.
  const userKnowledge = await readKnowledgeFile(userYgyDir())
  if (userKnowledge) {
    pushUniqueSection(
      `### User Preferences (~/.ygy-code/${userKnowledge.fileName})`,
