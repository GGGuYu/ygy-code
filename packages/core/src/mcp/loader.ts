// @ygy-code/core — MCP startup loader
//
...
import { YGY_DIR, debugLog } from '../utils.js'
...
export interface LoadOptions {
  /** mcpServers from ~/.ygy-code/config.json. Trusted implicitly. */
  userServers: Record<string, McpServerConfig> | undefined
  /** mcpServers from <project>/.ygy-code/config.json. Requires consent. */
...
export async function loadMcpFromDisk(opts: {
  ...
  const projectServers = await readMcpServersFromFile(path.join(opts.cwd, YGY_DIR, 'config.json'))
