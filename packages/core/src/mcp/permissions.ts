// @ygy-code/core — MCP tool permission gate
//
...
//     persisted per-tool to ~/.ygy-code/mcp-permissions.json, separate
...
import { debugLog, userYgyDir } from '../utils.js'

function permissionsFile(): string {
  return path.join(userYgyDir(), 'mcp-permissions.json')
}
...
export class McpPermissionStore {
  ...
  private async writePersisted(): Promise<void> {
    ...
    // ~/.ygy-code so practical leakage is limited to other apps running
    // as the same user).
