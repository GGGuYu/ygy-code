// @ygy-code/core — MCP project-level trust gate
//
// A `.ygy-code/config.json` checked into a git repo can declare MCP servers
...
//
// Persistence file: ~/.ygy-code/trusted-projects.json (mode 0600).
// Format: { trusted: [{ path: <absolute>, trustedAt: <ISO> }, ...] }
//
// User config (~/.ygy-code/config.json) is NOT subject to this gate —
...
function trustedFile(): string {
  return path.join(userYgyDir(), 'trusted-projects.json')
}
...
export async function promptForTrust(
  ...
  const answer = await askUser(question, [
    ...
    { label: 'Exit ygy-code', description: 'Close the CLI without loading any MCP servers.' },
  ])
