// @ygy-code/core — Reject env vars that are runtime code-injection vectors
//
// MCP stdio servers inherit an `env` map straight through to spawn(). That
// map can come from three sources:
//   1. the user typing `ygy mcp add --env KEY=VAL`
  ...
  constructor(public readonly key: string) {
    super(
      `Env key "${key}" is blocked by the MCP env safety check: it is a runtime ` +
        ...
        `really need this, export it in the shell that launches ygy instead.`,
    )
