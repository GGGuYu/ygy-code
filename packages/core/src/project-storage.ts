...
const GITIGNORE_ENTRY = '.ygy-code'
...
function alreadyIgnoresXcode(content: string): boolean {
  return content.split(/\r?\n/).some((line) => {
    ...
    return rule === '.ygy-code' || rule === '.ygy-code/' || rule === '/.ygy-code' || rule === '/.ygy-code/'
  })
}
 ...
 * this mkdir operation created the top-level .ygy-code directory. */
export async function ensureProjectStorageDir(directory: string): Promise<void> {
  try {
    ...
    if (createdDir !== undefined && path.basename(createdDir) === YGY_DIR) {
