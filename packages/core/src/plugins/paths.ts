// @ygy-code/core — Plugin filesystem layout
//
...
// `YGY_PLUGINS_DIR` test override has a single chokepoint.
//
// Layout (under ~/.ygy-code/plugins/ by default):
//
...
import { YGY_DIR, userYgyDir } from '../utils.js'
 ...
 *  - `YGY_PLUGINS_DIR` — plugin-specific override (parallels
 *    `YGY_AGENTS_DIR` / `YGY_SKILLS_DIR`; preferred when tests want to
 *    isolate JUST plugins without redirecting MCP / config / OAuth).
 *  - `YGY_CODE_HOME` — broad override of the whole `~/.ygy-code/` root
 *    (resolved via {@link userYgyDir}). Reroutes plugins along with
 *    config, MCP state, etc. */
export function pluginsRoot(): string {
  const override = process.env.YGY_PLUGINS_DIR
  if (override) return override
  return path.join(userYgyDir(), PLUGINS_DIR_NAME)
}

/** ~/.ygy-code/plugins/known_marketplaces.json */
export function knownMarketplacesPath(): string {
  return path.join(pluginsRoot(), 'known_marketplaces.json')
}

/** ~/.ygy-code/plugins/marketplaces/<name>/ */
...
/** ~/.ygy-code/plugins/marketplaces/<name>/marketplace.json */
export function marketplaceIndexPath(name: string): string {
  return path.join(marketplaceDir(name), 'marketplace.json')
}

/** ~/.ygy-code/plugins/cache/<marketplace>/<plugin>/ — all versions live
...
/** ~/.ygy-code/plugins/cache/<marketplace>/<plugin>/<version>/ */
...
/** ~/.ygy-code/plugins/data/<sanitised-plugin-id>/ — persistent per-plugin
...
/** ~/.ygy-code/plugins/installed_plugins.json */
export function installedPluginsPath(): string {
  return path.join(pluginsRoot(), 'installed_plugins.json')
}

/** <cwd>/.ygy-code/plugins/ — rare; used when a project ships its own
...
export function projectPluginsDir(cwd: string): string {
  return path.join(cwd, YGY_DIR, 'plugins')
}
 ...
 *  authored for Claude Code install in ygy-code without modification —
...
export const MANIFEST_CANDIDATES: ReadonlyArray<{ format: 'native' | 'claude' | 'bare'; rel: string }> = [
  { format: 'native', rel: '.ygy-code-plugin/plugin.json' },
