// @ygy-code/core — Plugin system core types
//
...
// in either CLI. The native `.ygy-code-plugin/plugin.json` path is also
// accepted (preferred for newly-authored ygy-code-only plugins) and a bare
 ...
 *
 *    'user'     →  ~/.ygy-code/settings.json
 *    'project'  →  <cwd>/.ygy-code/settings.local.json  (gitignored)
 *
 ...
 *  the shape of `mcpServers` in ~/.ygy-code/config.json — validated by
...
export interface PluginManifest {
   ...
   *  layout as `~/.ygy-code/skills/`). Or a single file path. */
  skills?: string
...
/** One entry in `~/.ygy-code/plugins/known_marketplaces.json` — a
...
// ── Installed plugin registry (~/.ygy-code/plugins/installed_plugins.json) ─
