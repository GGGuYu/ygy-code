// @ygy-code/core — Plugin startup loader
//
...
//
//   Pass 2 — project-local plugins under <cwd>/.ygy-code/plugins/<name>/.
...
export interface ResolvedContributions {
   ...
   *  ~/.ygy-code/config.json `mcpServers` shape). */
  ...
  try {
    ...
    if (!discovery) {
      args.errors.push({
        ...
        path: args.rootDir,
        message:
          'no plugin manifest found (looked for .ygy-code-plugin/plugin.json, .claude-plugin/plugin.json, plugin.json)',
      })
      return
    }
