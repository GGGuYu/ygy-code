// @ygy-code/core — Marketplace subscription + index parsing
//
 ...
 *
 *  Writes the parsed file to ~/.ygy-code/plugins/marketplaces/<name>/marketplace.json
 *  before returning. */
...
async function fetchViaShallowClone(source: string, signal?: AbortSignal): Promise<string> {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-marketplace-'))
  try {
