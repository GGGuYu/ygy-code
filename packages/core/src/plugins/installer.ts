// @ygy-code/core — Plugin installer
//
...
export async function installPlugin(req: InstallRequest): Promise<InstallResult> {
  ...
  // ~/.ygy-code/plugins/known_marketplaces.json. When the admin
    ...
    if (!subscribed) {
      throw new InstallError(
        `strict marketplace mode is enabled (known_marketplaces.json:strictKnownMarketplaces=true) — ` +
          ...
          `Either subscribe it first (\`ygy plugin marketplace add\`) or turn strict mode off.`,
      )
    }
  }
  ...
  try {
    ...
    if (!discovery) {
      throw new InstallError(
        'no plugin manifest found in source (looked for .ygy-code-plugin/plugin.json, .claude-plugin/plugin.json, plugin.json)',
      )
    }
    if (discovery.format === 'gemini') {
      throw new InstallError(
        'this is a Gemini extension (gemini-extension.json) — ygy-code does not support Gemini extensions; see docs/plugins.md',
      )
    }
...
async function fetchToTemp(source: PluginSource, signal?: AbortSignal): Promise<string> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugin-install-'))
  ...
  if (source.kind === 'local') {
    ...
    if (!stat || !stat.isDirectory()) {
      ...
      // process.cwd(), and when ygy is launched via `pnpm dev` the cwd
  ...
  if (source.kind === 'git' || source.kind === 'github') {
    ...
    if (subdir) {
      ...
      const subdirTemp = await fs.mkdtemp(path.join(os.tmpdir(), 'ygy-plugin-subdir-'))
      try {
 ...
 *  (`~/.ygy-code/plugins/data/<id>/`) intact so the user doesn't lose
