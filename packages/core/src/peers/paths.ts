...
export async function ensurePeerRuntimeDirectories(): Promise<{ registryDir: string; socketDir: string }> {
  const root = path.resolve(userYgyDir())
  ...
  const socketDir = path.join(os.tmpdir(), `ygy-code-peers-${uid}-${namespaceHash}`)
...
export function peerRegistryDir(): string {
  return path.join(path.resolve(userYgyDir()), 'runtime', 'peers')
}
