...
//
// User scope:     ~/.ygy-code/settings.json
// Project scope:  <repo-root>/.ygy-code/settings.local.json (gitignored)
//
...
export function skillSettingsPath(scope: SkillSettingsScope): string {
  ...
  return path.join(process.cwd(), YGY_DIR, 'settings.local.json')
}
