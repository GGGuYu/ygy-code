// @ygy-code/core — Skill loader
//
// Scans ~/.ygy-code/skills/*/SKILL.md and <repo-root>/.ygy-code/skills/*/SKILL.md
...
import { YGY_DIR, errorMessage, userYgyDir } from '../utils.js'
 ...
 *  Gemini CLI do the same listing at activation, but ygy-code caches it
 ...
 *  skill directories). Environment variable `YGY_SKILLS_DIR` overrides
...
export async function loadSkills(opts: LoadSkillsOptions = {}): Promise<SkillDefinition[]> {
  const override = process.env.YGY_SKILLS_DIR
  if (override) {
    const overrideSkills = await loadSkillsFromDir(override, 'project')
    return [...overrideSkills, ...(await loadFromExtras(opts.extraDirs))]
  }

  const userDir = path.join(userYgyDir(), 'skills')
  const projectDir = path.join(process.cwd(), YGY_DIR, 'skills')
...
async function loadFromExtras(extras: LoadSkillsOptions['extraDirs']): Promise<SkillDefinition[]> {
  ...
  for (const { dir, pluginId } of extras) {
    ...
    // dir under ~/.ygy-code/plugins/cache/..., which makes 'user' the
