...
export const OPENAI_CHATGPT_AUTH_RESPONSE_HEADER = 'x-ygy-code-openai-auth-mode'
export const OPENAI_CHATGPT_USAGE_LIMIT_HEADER = 'x-ygy-code-chatgpt-usage-limit'
...
function authenticatedHeaders(
  ...
  headers.set('originator', 'ygy-code')
...
async function normalizeChatGPTResponse(response: Response): Promise<Response> {
  ...
  const limitType = response.headers.get('ygy-codex-rate-limit-reached-type')?.toLowerCase() ?? ''
  const usageLimited =
...
export function createOpenAIChatGPTFetch(options: OpenAIChatGPTFetchOptions): typeof fetch {
  ...
  const userAgent = options.userAgent ?? 'ygy-code'
