...
// GPT-5.6 requires at least 0.144.0; sending ygy-code's independent 0.x version
...
function cachePath(): string {
  return path.join(userYgyDir(), 'cache', 'openai-chatgpt-models.json')
}
    ...
    fetchImpl(
      `${OPENAI_CHATGPT_OAUTH.modelsEndpoint}?client_version=${encodeURIComponent(OPENAI_CODEX_COMPATIBILITY_VERSION)}`,
      {
        headers: {
          ...
          originator: 'ygy-code',
          'User-Agent': `ygy-code/${clientVersion}`,
        },
        signal,
      },
    )
