// @ygy-code/core — Configuration resolution
//
...
//   2. `~/.ygy-code/config.json` `model` field — written by `/model` picker
//   3. `YGY_CODE_MODEL` environment variable
...
// restarts — otherwise a user who had `YGY_CODE_MODEL` set in their shell /
...
import { userYgyDir } from '../utils.js'
 ...
 *   2. `~/.ygy-code/config.json` `model` field (written by the /model picker)
 *   3. `YGY_CODE_MODEL` environment variable
...
export function resolveModelId(input?: string): string | null {
  const explicit = input ?? loadUserConfig().model ?? process.env.YGY_CODE_MODEL
  if (explicit) {
...
// ── User config file (~/.ygy-code/config.json) ────────────────────────────
//
 ...
 *  honour the YGY_CODE_HOME override automatically. */
export function getUserConfigPath(): string {
  return path.join(userYgyDir(), 'config.json')
}
...
export function saveUserConfig(update: Partial<UserConfig>): boolean {
  ...
  try {
    ...
    // YGY_CODE_HOME override creates `~/.ygy-code/` but writes to the override
    // and the write silently fails on a missing parent.
    fsSync.mkdirSync(userYgyDir(), { recursive: true })
