// @ygy-code/core — webSearch tool (multi-provider).
//
// Provider order: explicit YGY_CODE_WEB_SEARCH_PROVIDER override, then the
...
function pickProvider(modelProvider: string | undefined): PickResult {
  const forced = process.env.YGY_CODE_WEB_SEARCH_PROVIDER?.trim().toLowerCase()
  if (forced) {
    const p = ALL_PROVIDERS.find((candidate) => candidate.id === forced)
    if (!p) {
      return {
        error:
          `Unknown web search provider "${forced}" in YGY_CODE_WEB_SEARCH_PROVIDER. ` +
          `Valid values: ${ALL_PROVIDERS.map((candidate) => candidate.id).join(', ')}.`,
      }
    }
    if (!process.env[p.envKey]) {
      return { error: `YGY_CODE_WEB_SEARCH_PROVIDER=${forced} is set but ${p.envKey} is missing.` }
    }
    return { provider: p }
  }
